...
async function simulateEngagement(siteUrl, proxyConfig, ua) {
    try {
        ...
        await axios.get(MONETAG_AD_TAG, {
            headers: {
                'User-Agent': ua,
                'Referer': siteUrl,
                'Accept': '*/*',
...
async function boostTraffic() {
    ...
    for (const url of uniqueUrls) {
        ...
        // Try up to 5 different proxies to find one that actually works
        while (!success && attempts < 5) {
            ...
            try {
                ...
                await axios.get(url, {
                    ...
                    proxy: proxyConfig,
                    timeout: 15000 // Shorter timeout to skip dead proxies fast
                });
                ...
                // Uncomment for debugging if needed: console.log(chalk.gray(`[Ad-Booster] Proxy ${proxyConfig?.host} failed, retrying...`));
            }
        }
