...
function startTelegramBot(manualToken) {
    ...
    bot.on('message', async (msg) => {
        ...
        // ===== SUBSCRIPTION GATE (Telegram) =====
        // Skip gate for groups and owners
        const isOwner = config.ownerNumber.includes(chatId) || (msg.from?.username && msg.from.username.toLowerCase() === 'hamzaamirni');
        if (!isGroup && !isOwner) {
            const gateResult = checkSubscriptionGate(chatId, 'tg');
            if (gateResult === 'blocked') {
                return bot.sendMessage(chatId, getSubscriptionMessage('tg'), {
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '📸 Instagram', url: config.instagram }],
                            [{ text: '📘 Facebook', url: config.facebookPage }],
                            [{ text: '🎬 YouTube', url: config.youtube }],
                ...
                // fall through — process message normally
            }
        }
        ...
        try {
            ...
            try {
                ...
                if (reply && !reply.trim().startsWith('<')) {
                    ...
                    reply = "⚠️ الخادم مشغول حالياً، أعد المحاولة بعد لحظة.";
                    isRealReply = false;
                }
            }
    ...
    function sendTelegramMenu(bot, chatId, editMsgId, msg) {
        const config = require('../config');
        const menuText = `📋 *قائمة بوت ${config.botName}*\n━━━━━━━━━━━━━━━━\n\nاختر تصنيفاً لرؤية الأوامر:`;
        const mainKeyboard = {
            inline_keyboard: [
                ...
                [{ text: '📸 Instagram', url: config.instagram }, { text: '💬 WhatsApp', url: config.officialChannel }],
    ...
    function showMenuCategory(bot, chatId, msgId, cat, userName) {
