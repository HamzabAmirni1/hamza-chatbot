...
async function getGeminiProxyVision(base64, prompt, mime = "image/jpeg") {
    try {
        console.log(chalk.blue(`[Vision] Starting Gemini Proxy branch...`));
        ...
        console.error("Gemini Proxy Error:", e.response?.data || e.message);
        return null;
    }
}
...
async function getOpenRouterResponse(jid, text, imageBuffer = null) {
    try {
        ...
        const { data } = await axios.post(
            ...
            {
                model: imageBuffer ? "meta-llama/llama-3.2-11b-vision-instruct:free" : "meta-llama/llama-3.1-8b-instruct:free",
                messages,
            },
            {
                headers: {
...
async function getGeminiResponse(jid, prompt, imageBuffer = null, mime = "image/jpeg", customApiKey = null) {
    try {
        const apiKey = customApiKey || config.geminiApiKey || "AIzaSyB3Q74etnADQ_qSX3OJtzTnteGh-fd4df8";
...
async function analyzeImage(buffer, mime, prompt, directUrl = null) {
    try {
        ...
        // 0. Gemini Proxy Staging (Free, keyless and highly reliable fallback)
        promises.push(getGeminiProxyVision(base64, finalPrompt, mime));
        ...
        promises.push(getGeminiResponse("system", finalPrompt, buffer, mime, geminiKey));
        ...
        // 4.5 Pollinations Direct Base64 — run 3 models in parallel for best chance
        const pollinationsModels = ['openai', 'openai-large', 'mistral'];
        for (const model of pollinationsModels) {
            promises.push((async (m) => {
                try {
                ...
                throw new Error(`Pollinations ${m} base64 failed`);
            })(model));
        }
