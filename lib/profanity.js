...
async function handleProfanity(platform, senderJid, senderName, text, matchedWord, sockOrBot, msgObj) {
    try {
        ...
        // Send notification message in dev_messages table
        await db.saveDevMessage({
            id: `profanity-${Date.now()}`,
            sender: senderJid,
            senderName: `⚠️ كشف كلام بذيء: ${senderName || 'مستخدم'}`,
            platform: platform,
            text: `استخدم كلمة بذيئة: "${matchedWord}" في الرسالة: "${text}". بقي له ${warningsLeft} تحذيرات.`,
            timestamp: new Date().toISOString()
        });
