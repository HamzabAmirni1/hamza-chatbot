...
async function sendFacebookImage(recipientId, imageBuffer, caption, pageToken = config.fbPageAccessToken) {
    ...
    await sendFacebookMedia(recipientId, imageBuffer, 'image', caption, pageToken);
}
    ...
    if (isBuffer) {
        try {
            ...
            if (!attachmentId && type === 'audio') {
                ...
                if (attachmentId) {
                    await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${pageToken}`, {
                        recipient: { id: recipientId },
                        message: { attachment: { type: 'file', payload: { attachment_id: attachmentId } } }
                    });
            ...
            if (attachmentId) {
                ...
                await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${pageToken}`, {
                    recipient: { id: recipientId },
                    message: { attachment: { type, payload: { attachment_id: attachmentId } } }
                });
                if (caption) await sendFacebookMessage(recipientId, caption, pageToken);
                return;
            }
    ...
    if (isBuffer && global.storeTempMedia) {
        try {
            ...
            try {
                await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${pageToken}`, {
                    recipient: { id: recipientId },
                    message: { attachment: { type, payload: { url: selfUrl, is_reusable: false } } }
                });
                if (caption) await sendFacebookMessage(recipientId, caption, pageToken);
                return;
            } catch (fbErr) {
                ...
                if (type === 'audio') {
                    ...
                    try {
                        await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${pageToken}`, {
                            recipient: { id: recipientId },
                            message: { attachment: { type: 'file', payload: { url: selfUrl, is_reusable: false } } }
                        });
                        if (caption) await sendFacebookMessage(recipientId, caption, pageToken);
                        return;
                    } catch (innerErr) {
            ...
            if (cdnUrl) {
                ...
                try {
                    ...
                    if (type === 'audio') {
                        ...
                        await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${pageToken}`, {
                            recipient: { id: recipientId },
                            message: { attachment: { type: 'file', payload: { url: cdnUrl, is_reusable: true } } }
                        });
                        if (caption) await sendFacebookMessage(recipientId, caption, pageToken);
                        return;
                    }
...
async function handleFacebookMessage(event) {
    try {
        ...
        // Media received → silently download and save to context history
        // If no command attached → stop here (no auto-reply)
        // If command is attached (e.g. photo + .analyze) → save image then fall through to command execution
        if (isImage || isVideo) {
            try {
                if (mediaUrl) {
            ...
            if (!isCommand) return; // No command → stop here, no auto-reply
...
module.exports = { handleFacebookMessage, sendFacebookMessage, sendFacebookMedia, saveFbUser, getFbUsers, fetchFbProfileName };
