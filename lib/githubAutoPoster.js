...
async function getCache() {
    try {
        const cache = await db.getCache('github');
        if (cache && Array.isArray(cache)) return { lastRepos: cache };
    } catch(e) {}
    fs.ensureDirSync(DATA_DIR);
    if (!fs.existsSync(GITHUB_CACHE_FILE)) return { lastRepos: [] };
    try { return fs.readJsonSync(GITHUB_CACHE_FILE); } catch(e) { return { lastRepos: [] }; }
}

async function saveCache(cache) {
    try { await db.setCache('github', cache.lastRepos || []); } catch(e) {}
    try { fs.writeJsonSync(GITHUB_CACHE_FILE, cache); } catch(e) {}
}
...
async function checkAndPostTrending() {
    if (config.enableGithubAutoPoster !== 'true') {
        console.log("[githubAutoPoster] GitHub Auto-Poster is disabled.");
        return;
    }
    const moment = require('moment-timezone');
    const hour = moment().tz(config.timezone || 'Africa/Casablanca').hour();
    if (hour < 8 || hour > 22) {
        console.log("[githubAutoPoster] Outside active posting hours (8am - 10pm). Skipping trending check.");
        return;
    }
