...
// API Airforce — Free GPT-4o-mini, no auth required (works from cloud servers)
async function getAirforceAI(jid, message) {
    try {
        ...
        return reply ? sanitizeAiResponse(reply) : null;
    } catch (e) {
        console.error(`[Airforce AI Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}

// Akash Network — Free Llama 3.1 8B, no auth required
async function getAkashAI(jid, message) {
    try {
        ...
        const reply = data?.choices?.[0]?.message?.content;
        if (reply) {
            console.log(`[Akash AI] Success`);
            return sanitizeAiResponse(reply);
        }
        return null;
    } catch (e) {
        console.error(`[Akash AI Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}

// GitHub Models — Free GPT-4o-mini via GitHub token (no billing required)
async function getGithubModelsAI(jid, message) {
    try {
        ...
        console.error(`[GitHub Models Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}


// Open AI-compatible APIs — rotating through multiple free provider base URLs
async function getAIChartResponse(jid, message) {
    const providers = [
        'https://api.openai-hk.com/v1',
        'https://chatapi.akash.network/api/v1',
    ];
    const models = ['gpt-4o-mini', 'Meta-Llama-3-1-8B-Instruct-FP8'];
    for (let i = 0; i < providers.length; i++) {
        try {
            const { data } = await axios.post(`${providers[i]}/chat/completions`, {
                model: models[i] || 'gpt-4o-mini',
                messages: [
                    { role: 'system', content: getSystemPrompt(message) },
                    { role: 'user', content: message }
                ],
                ...
                headers: {
                    'Authorization': 'Bearer hk-no-key',
                    'Content-Type': 'application/json'
                },
                timeout: 12000
            });
            const reply = data?.choices?.[0]?.message?.content;
            if (reply) return sanitizeAiResponse(reply);
        } catch (e) {
            console.error(`[AIChart ${providers[i]} Error]: ${e.message} (Status: ${e.response?.status})`);
        }
    }
    return null;
}
