...
/**
 * Fix unbalanced Markdown markers (*bold*, _italic_) before sending to Telegram.
 * Telegram rejects messages with unclosed entities with "can't parse entities" error.
 */
function sanitizeTelegramMarkdown(text) {
    if (!text || typeof text !== 'string') return text;

    // Fix broken [text](url) markdown links → plain "text: url"
    text = text.replace(/\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/gi, (_, label, url) => {
        if (label.startsWith('http') || label.trim() === url.trim()) return url;
        return `${label.trim()}: ${url}`;
    });

    // Balance * (bold) markers — count them, if odd strip all bold markers
    const boldCount = (text.match(/\*/g) || []).length;
    if (boldCount % 2 !== 0) {
        text = text.replace(/\*/g, '');
    }

    // Balance _ (italic) markers — count them, if odd strip all italic markers
    const italicCount = (text.match(/_/g) || []).length;
    if (italicCount % 2 !== 0) {
        text = text.replace(/_/g, '');
    }

    // Balance ` (code/mono) markers
...
/**
 * Safe Telegram sendMessage — tries Markdown first, falls back to plain text.
 */
async function safeTgSend(bot, chatId, text, extraOpts = {}) {
    const clean = sanitizeTelegramMarkdown(text);
    try {
        return await bot.sendMessage(chatId, clean, { parse_mode: 'Markdown', ...extraOpts });
    } catch (e) {
        // Fallback: send as plain text (no parse_mode)
        try {
            return await bot.sendMessage(chatId, clean, { ...extraOpts });
        } catch (e2) {
            console.error(`[TG] safeTgSend failed: ${e2.message}`);
        }
    }
}
...
async function saveTelegramUser(chatId) {
    let isNew = false;
    try {
        ...
        if (!users.includes(id)) {
            // If not found locally, check if they exist in Supabase DB
            const existsInDb = await db.hasUser(`tg:${chatId}`);
            ...
            if (!existsInDb) {
                ...
                try {
                    ...
                    if (!isSubscribed(id, 'tg')) {
                        subscribeUser(id, config.defaultPrayerCity || 'Casablanca', 'MA', 'tg');
                        console.log(`[Prayer] Auto-subscribed new TG user ${id} to prayer reminders (Casablanca)`);
                    }
                } catch (e) {}
            }
        }
...
function startTelegramBot(manualToken) {
    ...
    bot.on('message', async (msg) => {
        ...
        if (msg.from.is_bot) return;
            ...
            if (global.bannedUsersCache && (global.bannedUsersCache.includes(`tg:${chatId}`) || global.bannedUsersCache.includes(chatId))) {
                const isMsgToDev = text && (text.trim().startsWith('/msgtodev') || text.trim().startsWith('.msgtodev') || text.trim().startsWith('!msgtodev'));
                if (!isMsgToDev) {
        ...
        // Track Telegram user + auto-subscribe prayer
        const isNewTgUser = await saveTelegramUser(chatId);

        // ── Welcome message for NEW Telegram users ──
        if (isNewTgUser) {
            try {
                const { getUserCity } = require('./prayerScheduler');
                const defaultCity = getUserCity(chatId.toString(), 'tg') || 'Casablanca';
                await bot.sendMessage(chatId,
                    `🌙 *أهلاً وسهلاً بك في ${config.botName}!* 🎉\n\n` +
                    `━━━━━━━━━━━━━━━━━━\n` +
                    `🕌 *تم تفعيل تذكير أوقات الصلاة تلقائياً!*\n` +
                    `📍 مدينتك الافتراضية: *${defaultCity}*\n\n` +
                    `يمكنك تغيير مدينتك في أي وقت:\n` +
                    `📝 /salat Marrakech — لضبط مدينتك\n` +
                    ...
                    `اكتب /help لعرض قائمة الأوامر 🚀`,
                    {
                        ...
                        reply_markup: {
                            inline_keyboard: [
                                [
                                    { text: '🕌 أوقات الصلاة', callback_data: 'salat_now' },
                                    { text: '🌍 تغيير مدينتي', callback_data: 'salat_city' }
                                ],
                                [
                                    { text: '🔕 إيقاف التذكير', callback_data: 'salat_off' },
                                    { text: '📋 قائمة الأوامر', callback_data: 'cmd_help' }
                                ]
                            ]
                        }
                    }
        ...
        const fullName = `${msg.from.first_name || ''} ${msg.from.last_name || ''}`.trim() || `مستخدم تيليغرام (${cleanId})`;
        ...
        if (msg.chat.type === 'private') {
            const isMsgToDev = text && (text.trim().startsWith('/msgtodev') || text.trim().startsWith('.msgtodev') || text.trim().startsWith('!msgtodev'));
            if (!isMsgToDev) {
                const gateStatus = checkSubscriptionGate(chatId, 'tg');
                if (gateStatus === 'blocked') {
                    await bot.sendMessage(chatId, getSubscriptionMessage('tg'), { parse_mode: 'Markdown' });
                    return;
                } else if (gateStatus === 'pending') {
                    await bot.sendMessage(chatId, getWelcomeMessage(), { parse_mode: 'Markdown' });
                }
            }
            // Auto-subscribe to daily reminders & prayers
            const { autoSubscribeUser } = require('./subscription');
            autoSubscribeUser(chatId, 'tg');
        }
        ...
        const isOwner = config.ownerNumber && config.ownerNumber.some(n => n.replace(/[^0-9]/g, '') === userId.toString());
        const isAdmin = isGroup ? await checkTGAdmin(bot, chatId, userId) : false;

        // Check for profanity / bad language
        if (text && !isCommand && !isAdmin && !isOwner) {
            ...
            if (matchedIbhaya) {
                await handleIbhaya('TG', chatId.toString(), msg.from?.first_name || 'مستخدم', text, matchedIbhaya, bot, msg);
                return;
            }
        }
        ...
        // Photo/Video received → silently save to context history; download and auto-analyze/reply to photos if enabled
                ...
                if (buffer && msg.photo) {
                    ...
                    if (chatbotEnabled) {
                        ...
                        if (shouldReply) {
                            ...
                            // Get Telegram CDN URL directly (skip upload)
                            let tgFileUrl = null;
                            try {
                                const fileId = msg.photo[msg.photo.length - 1].file_id;
                                tgFileUrl = await bot.getFileLink(fileId);
                            } catch (_) {}
                            const replyText = await ai.analyzeImage(buffer, 'image/jpeg', finalPrompt, tgFileUrl);
                            if (replyText) {
                                await bot.sendMessage(chatId, replyText, { reply_to_message_id: msg.message_id });
                                try { await addToHistory('tg:' + chatId, "assistant", replyText); } catch (_) {}
                            }
                        }
                    }
                }
            } catch (e) {
                console.error('[Telegram Media Process/Reply Error]:', e.message);
            }
        ...
        try {
                ...
                if (allCmds[command]) {
                    try {
                        ...
                        if (!limitCheck.allowed) {
                            await bot.sendMessage(chatId, limitCheck.message, { parse_mode: 'Markdown' });
                            commandHandled = true;
                            return;
                        }
                        ...
                        const mockSock = createMockSock(bot, msg, chatId);
                        const helpers = { isTelegram: true, command: command, telegramToken: token };
                        await cmdFile(mockSock, chatId, msg, args, helpers, detectLanguage(text));
                        commandHandled = true;
                        await rateLimiter.incrementUsage(jid, command);
                ...
                for (const [key, path] of Object.entries(nlcKeywords)) {
                    ...
                    if (new RegExp(`(^|\\s)(${key})(\\s|$)`, "i").test(lowerBody)) {
                        try {
                            ...
                            const mockSock = createMockSock(bot, msg, chatId);
                            const helpers = { isTelegram: true, command: path.split('/').pop(), telegramToken: token };
            ...
            try {
                ...
                console.log(chalk.cyan(`[TG AI] Asking Gemini Engine (Web RPC + Official Keys)...`));
                reply = await getGeminiResponse(tgJid, text);
                if (reply) isRealReply = true;

                // 2. Try OpenRouter if configured
                if (!reply && config.openRouterKey) {
                    console.log(chalk.cyan(`[TG AI] Trying OpenRouter...`));
                    reply = await getOpenRouterResponse(tgJid, text);
                    if (reply) isRealReply = true;
                }

                // 3. Try ChatEverywhere (GPT-4) — backup
                if (!reply) {
                    console.log(chalk.cyan(`[TG AI] Trying ChatEverywhere (GPT-4)...`));
                    reply = await Promise.race([
                        getChatEverywhereAI(tgJid, text),
                ...
                if (!reply) {
                    console.log(chalk.cyan(`[TG AI] Racing fallback models...`));
                    reply = await Promise.race([
                        Promise.any([
                            getAkashAI(tgJid, text),
                            getChatUpAI(tgJid, text),
                            getNowTechAI(tgJid, text),
                ...
                console.error(`[TG AI] Error in sequential flow: ${e.message}`);
            }
            ...
            if (reply) {
                ...
                if (botReplyText) {
                    await safeTgSend(bot, chatId, botReplyText, {
                        reply_markup: {
                    ...
                    if (cmdMatch) {
                        ...
                        if (allCmds[command]) {
                            try {
                                ...
                                const cmdFile = require(`../commands/${allCmds[command]}`);
                                const mockSock = createMockSock(bot, msg, chatId);
                                await cmdFile(mockSock, chatId, msg, args, { isTelegram: true, command: command, telegramToken: token }, detectLanguage(text));
                                await rateLimiter.incrementUsage(jid, command);
                                if (global.trackCommand) global.trackCommand(command, 'telegram');
                            } catch (err) {
    ...
    bot.on('callback_query', async (query) => {
        ...
        try {
            ...
            if (data.startsWith('menu_cmd_')) {
                ...
                if (allCmds[cmd]) {
                    try {
                        ...
                        await cmdFile(mockSock, chatId, dummyMsg, [], { isTelegram: true, command: cmd, telegramToken: token }, 'ar');
            ...
            // ── Prayer button callbacks (from welcome message) ──
            if (data === 'salat_now' || data === 'salat_off') {
                await bot.answerCallbackQuery(query.id);
                const cmdText = data === 'salat_now' ? '.salat now' : '.salat off';
                const dummyMsg2 = { ...query.message, from: query.from, text: cmdText, key: { fromMe: false, id: query.message.message_id.toString(), remoteJid: chatId }, pushName: query.from.first_name };
                bot.emit('message', dummyMsg2);
                return;
            }
            if (data === 'salat_city') {
                await bot.answerCallbackQuery(query.id, { text: '📝 أرسل: .salat [اسم المدينة]' });
                await bot.sendMessage(chatId,
                    `🌍 *تغيير مدينتك لتذكير الصلاة:*\n\nأرسل اسم مدينتك بالإنجليزية، مثلاً:\n• \`.salat Casablanca\`\n• \`.salat Marrakech\`\n• \`.salat Fes\`\n• \`.salat Rabat\`\n• \`.salat Agadir\`\n• \`.salat Tangier\`\n• \`.salat Meknes\`\n• \`.salat Oujda\``,
                    { parse_mode: 'Markdown' }
                );
                return;
            }
            if (data === 'cmd_help') {
                await bot.answerCallbackQuery(query.id);
                const dummyMsg3 = { ...query.message, from: query.from, text: '.help', key: { fromMe: false, id: query.message.message_id.toString(), remoteJid: chatId }, pushName: query.from.first_name };
                bot.emit('message', dummyMsg3);
                return;
            }
