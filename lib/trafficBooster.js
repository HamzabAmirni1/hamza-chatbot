...
async function boostTraffic() {
    if (isPaused || config.enableTrafficBooster !== 'true') return; // Traffic is OFF
    
    // Pick target URL dynamically from config
    const urls = config.trafficUrls || [MAIN_SITE];
    const targetUrl = urls[Math.floor(Math.random() * urls.length)] || MAIN_SITE;
    ...
    try {
        ...
        // Append utm suffix only for main site visits
        const urlToVisit = (targetUrl.includes('netlify.app'))
            ? `${targetUrl}${targetUrl.includes('?') ? '&' : '?'}${utmSuffix.slice(1)}`
            : targetUrl;

        await axios.get(urlToVisit, {
            headers: {
                'User-Agent': ua,
                'Referer': source.ref,
                'Accept-Language': geo.lang,
        ...
        visitCounter++;
        console.log(chalk.blue(`[Ad-Booster] 🚀 Visit #${visitCounter} to [${urlToVisit}] via ${source.source}`));

        // 2. Fire GA4 & Monetag pings only if target URL is netlify main site
        if (targetUrl.includes('netlify.app')) {
...
async function startTrafficInterval() {
    ...
    console.log(chalk.green('🔥 Ad-Booster v8.0 — Lightweight Axios (No Chromium). Dynamic intervals enabled.'));
    ...
    // 6 staggered workers — delay is based dynamically on trafficIntervalMinutes config
    const runWorker = async (delayMs) => {
        const run = async () => {
            try { await boostTraffic(); await checkMilestones(); } catch (_) { }
            
            const intervalMin = parseFloat(config.trafficIntervalMinutes || '5');
            const baseMs = intervalMin * 60000;
            // Introduce +/- 20% randomness to simulate organic user arrival
            const nextDelay = baseMs + (Math.random() * 0.4 - 0.2) * baseMs;
            setTimeout(run, Math.max(15000, nextDelay));
        };
        setTimeout(run, delayMs);
    };

    // Stagger 6 workers over the initial interval
    const intervalMin = parseFloat(config.trafficIntervalMinutes || '5');
    const baseMs = intervalMin * 60000;
    for (let i = 0; i < 6; i++) {
        runWorker(i * (baseMs / 6));
    }
}
