        ...
        try {
            ...
            if (needsWebSearch(text)) {
                try {
                    ...
                    console.log(chalk.cyan(`[TG WebSearch] Searching for: "${text.substring(0, 60)}"`));
                    ...
                    if (webCtx) {
                        ...
                        console.log(chalk.green(`[TG WebSearch] Injected ${webCtx.length} chars into AI context`));
                    }
                } catch (e) {}
            }

            const aiPromises = [
                getPollinationsResponse(tgJid, aiText),
                getStableAIResponse(tgJid, aiText),
                getDuckDuckGoAI(tgJid, aiText),
                getTypeGPTResponse(tgJid, aiText),
                getNerimityAI(tgJid, aiText),
            ];
            if (config.geminiApiKey) aiPromises.push(getGeminiResponse(tgJid, aiText));
            if (config.openRouterKey) aiPromises.push(getOpenRouterResponse(tgJid, aiText));
            aiPromises.push(getAutoGPTResponse(tgJid, aiText));
            ...
            try {
                ...
                console.log(chalk.yellow(`[TG AI] Race failed with web context. Retrying with plain text...`));
                // Retry with original plain text (no web injection)
                const fallbackPromises = [
                    getDuckDuckGoAI(tgJid, text),
                    getTypeGPTResponse(tgJid, text),
                    getNerimityAI(tgJid, text),
                    getPollinationsResponse(tgJid, text),
                    getStableAIResponse(tgJid, text),
                    getAutoGPTResponse(tgJid, text),
                ];
                if (config.geminiApiKey) fallbackPromises.push(getGeminiResponse(tgJid, text));
                try {
                    reply = await Promise.race([
                        Promise.any(fallbackPromises.map(p => p.then(r => {
                            if (!r || r.trim().startsWith('<')) throw new Error('bad');
                            return r;
                        }))),
                        new Promise((_, rej) => setTimeout(() => rej(), 15000))
                    ]);
                    if (reply) { isRealReply = true; console.log(chalk.green(`[TG AI] Plain-text retry succeeded`)); }
                } catch (_) {}
                if (!reply) {
