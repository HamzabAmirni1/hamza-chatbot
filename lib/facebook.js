...
async function handleFacebookMessage(event) {
    try {
        ...
        try {
            ...
            try {
                // 1. Try Gemini Web / Official / DuckDuckGo Smart Resolver (Fast & Keyless)
                console.log(chalk.cyan(`[FB AI] Asking Gemini Engine (Web RPC + Official Keys)...`));
                ...
                // 3. Try ChatEverywhere (GPT-4) — backup
                if (!reply) {
                    console.log(chalk.cyan(`[FB AI] Trying ChatEverywhere (GPT-4)...`));
                ...
                // 4. Race fallback providers in parallel
                if (!reply) {
                    console.log(chalk.cyan(`[FB AI] Racing fallback models...`));
                    reply = await Promise.race([
                        Promise.any([
                            getAkashAI(fbJid, text),
                            getChatUpAI(fbJid, text),
                            getNowTechAI(fbJid, text),
                            getPollinationsResponse(fbJid, text),
