...
ايلى بغيتي تصاوب شي بوت بحالي ولا عندك مشروع ويب، تواصل مع حمزة نيشان! ✨`;
                ...
                if (body && (body.toLowerCase() === '.menu' || body.toLowerCase() === '.help' || body.toLowerCase() === 'menu' || body.toLowerCase() === 'help' || body.toLowerCase() === 'قائمة' || body.toLowerCase() === '.قائمة')) {
                    const sections = [
                        {
                            title: "🛠️ أدوات الصور (AI IMAGE)",
                            rows: [
                                { title: "🪄 تعديل سحري (.nano)", rowId: ".nano", description: "تعديل الصور بالذكاء الاصطناعي" },
                                { title: "✨ تحسين الجودة (.hd)", rowId: ".hd", description: "رفع جودة الصور المتوشوشة" },
                                { title: "🖼️ إزالة الخلفية (.bg)", rowId: ".bg", description: "مسح الخلفية من أي صورة" },
                                { title: "🎨 الرسم الذكي (.draw)", rowId: ".draw", description: "ارسم أي حاجة بتخيلك" },
                                { title: "🧠 تحليل الصور (.hl)", rowId: ".hl", description: "شرح محتوى الصور" }
                            ]
                        },
                        {
                            title: "🤖 نماذج الدردشة (AI CHAT)",
                            rows: [
                                { title: "🤖 GPT-4o", rowId: ".gpt4o", description: "أذكى نموذج من OpenAI" },
                                { title: "⚡ GPT-4o Mini", rowId: ".gpt4om", description: "سريع واقتصادي" },
                                { title: "🧠 OpenAI O1", rowId: ".o1", description: "للمسائل المعقدة" },
                                { title: "💬 Auto-Reply", rowId: "auto", description: "البوت كايجاوبك بلا أوامر!" }
                            ]
                        },
                        {
                            title: "📡 خدمات وأرقام (SERVICES)",
                            rows: [
                                { title: "📱 أرقام وهمية (.tempnum)", rowId: ".tempnum", description: "أرقام لتفعيل الحسابات" },
                                { title: "🔍 بحث يوتيوب (.yts)", rowId: ".yts", description: "بحث وتحميل الفيديوهات" },
                                { title: "🏓 سرعة البوت (.ping)", rowId: ".ping", description: "فحص سرعة استجابة السيرفر" }
                            ]
                        },
                        {
                            title: "📱 تواصل مع المطور (OWNER)",
                            rows: [
                                { title: "📸 Instagram", rowId: ".ig", description: "حسابي على إنستغرام" },
                                { title: "📺 YouTube", rowId: ".yt", description: "قناتي التعليمية" },
                                { title: "✈️ Telegram", rowId: ".tg", description: "قناتي على تليغرام" },
                                { title: "📢 WA Channel", rowId: ".channel", description: "قناة تحديثات البوت" },
                                { title: "🌐 Portfolio", rowId: ".web", description: "موقعي الشخصي" }
                            ]
                        }
                    ];
...
📌 *Uptime:* ${getUptime()}`;
                    ...
                    await sock.sendMessage(sender, {
                        text: menuText,
                        footer: `⚔️ ${config.botName} • Active 24/7 on Koyeb`,
                        title: "📜 قائمة الخدمات المتوفرة",
                        buttonText: "فتح القائمة 🛠️",
                        sections: sections,
                    ...
                    try {
                        ...
                        const rows = videos.map((v, i) => {
                            ...
                            return {
                                ...
                                rowId: `.video ${v.url}`,
                                description: `⏱️ ${v.timestamp} • 👁️ ${v.views.toLocaleString()}`
                            };
                        });
                        ...
                        await sock.sendMessage(sender, {
                            text: resultText,
                            footer: `⚔️ ${config.botOwner} • ${config.botName}`,
                            title: `🎬 نتائج البحث عن: ${searchQuery}`,
                            buttonText: 'قائمة الفيديوهات 🎬',
                            sections: [{
                                title: 'اختر فيديو للتحميل',
                                rows: rows
                            }]
                        }, { quoted: msg });
                ...
                if (body && body.toLowerCase() === '.tempnum') {
                    ...
                    try {
                        ...
                        // Format rows for list message
                        const rows = results.slice(0, 20).map((res, i) => ({
                            title: `${res.country} - ${res.number}`,
                            rowId: `.getsms ${res.link}`,
                            description: `المصدر: ${res.source}`
                        }));

                        let listTitle = `📱 *أرقام وهمية لتفعيل الحسابات (7sim)*\n\n💡 اختر رقماً من القائمة بالضغط على الزر أسفله لمشاهدة الرسائل المستلمة.`;
                        ...
                        await sock.sendMessage(sender, {
                            ...
                            title: "🌍 قائمة الأرقام الوهمية",
                            buttonText: "اختيار رقم 📱",
                            sections: [{
                                title: "الأرقام المتوفرة حالياً",
                                rows: rows
                            }]
                        }, { quoted: msg });
