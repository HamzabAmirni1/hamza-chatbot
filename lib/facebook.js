...
const { getContext, addToHistory, getAutoGPTResponse, getGeminiResponse, getLuminAIResponse, getAIDEVResponse, getPollinationsResponse, getBlackboxResponse, getStableAIResponse, getOpenRouterResponse, detectLanguage, webSearch, needsWebSearch, getDuckDuckGoAI, getTypeGPTResponse, getNerimityAI } = require('./ai');
    ...
    try {
        ...
        for (const chunk of chunks) {
            ...
            const payload = {
                ...
                messaging_type: 'RESPONSE',  // Standard response type (no deprecated tags)
                message: { text: chunk }
            };
            // NOTE: ACCOUNT_UPDATE, CONFIRMED_EVENT_UPDATE, POST_PURCHASE_UPDATE tags
            // were deprecated by Meta on April 27, 2026 (error_subcode 1893061).
            // We no longer use MESSAGE_TAG. All messages go as RESPONSE type.
...
async function handleFacebookMessage(event) {
    try {
        ...
        try {
            ...
            // ── Web Search Enrichment (inject live context for ALL providers) ──
            let aiText = text;
            if (needsWebSearch(text)) {
                try {
                    const lang = detectLanguage(text);
                    console.log(chalk.cyan(`[FB WebSearch] Searching for: "${text.substring(0, 60)}"`));
                    const webCtx = await Promise.race([
                        webSearch(text, lang),
                        new Promise(r => setTimeout(() => r(null), 5000))
                    ]);
                    if (webCtx) {
                        aiText = `${webCtx}\n\n---\nبناءً على المعلومات أعلاه من الإنترنت، أجب عن هذا السؤال:\n${text}`;
                        console.log(chalk.green(`[FB WebSearch] Injected ${webCtx.length} chars into AI context`));
                    }
                } catch (e) {}
            }
            ...
            if (config.geminiApiKey) aiPromises.push(getGeminiResponse(fbJid, aiText));
            if (config.openRouterKey) aiPromises.push(getOpenRouterResponse(fbJid, aiText));

            aiPromises.push(getDuckDuckGoAI(fbJid, aiText));
            aiPromises.push(getTypeGPTResponse(fbJid, aiText));
            aiPromises.push(getNerimityAI(fbJid, aiText));
            aiPromises.push(getLuminAIResponse(fbJid, aiText));
            aiPromises.push(getAIDEVResponse(fbJid, aiText));
            aiPromises.push(getPollinationsResponse(fbJid, aiText));
            aiPromises.push(getBlackboxResponse(fbJid, aiText));
            aiPromises.push(getStableAIResponse(fbJid, aiText));
            aiPromises.push(getAutoGPTResponse(fbJid, aiText));
            ...
            try {
                ...
                console.log(chalk.yellow(`[FB AI] Race failed with web context. Retrying with plain text...`));
                // Retry with original plain text (no web injection)
                const fallbackPromises = [];
                if (config.geminiApiKey) fallbackPromises.push(getGeminiResponse(fbJid, text));
                fallbackPromises.push(getDuckDuckGoAI(fbJid, text));
                fallbackPromises.push(getTypeGPTResponse(fbJid, text));
                fallbackPromises.push(getNerimityAI(fbJid, text));
                fallbackPromises.push(getPollinationsResponse(fbJid, text));
                fallbackPromises.push(getStableAIResponse(fbJid, text));
                fallbackPromises.push(getAutoGPTResponse(fbJid, text));
                try {
                    reply = await Promise.race([
                        Promise.any(fallbackPromises.map(p => p.then(r => { if (!r) throw new Error('no'); return r; }))),
                        new Promise((_, rej) => setTimeout(() => rej(), 15000))
                    ]);
                    if (reply) console.log(chalk.green(`[FB AI] Plain-text retry succeeded`));
                } catch (_) {}
                if (!reply) {
                    reply = await getStableAIResponse(fbJid, text) || await getPollinationsResponse(fbJid, text);
                    if (reply) {
