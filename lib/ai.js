...
function getSystemPrompt(text, isVision = false) {
    ...
    if (isVision) {
        return lang === 'ar' 
            ? "أنت مساعد ذكي خبير في تحليل الصور. اشرح ما تراه بدقة وباللغة العربية أو الدارجة." 
            : "You are an expert image analyzer. Describe the image in detail in English.";
    }
...
async function analyzeImage(buffer, mime, prompt) {
    try {
        ...
        // 3. Obito Vision API & Free Gemini API
        promises.push((async () => {
            console.log(chalk.blue(`[Vision] Starting Pollinations branch...`));
            // Branch C: Pollinations AI (Using Base64 directly, bypassing upload errors)
            try {
                ...
                if (typeof data === 'string' && !data.includes("<!doctype")) {
                    console.log(chalk.green(`[Vision] Pollinations succeeded`));
                    return data;
                }
                console.log(chalk.yellow(`[Vision] Pollinations returned invalid data`));
            } catch (e) {
                console.log(chalk.yellow(`[Vision] Pollinations failed: ${e.message}`));
            }

            // If base64 fails, fallback to uploading for Siputzx/Ryzendesu
            console.log(chalk.blue(`[Vision] Attempting media upload for fallback APIs...`));
            const { uploadToCatbox, uploadToTelegraPh, uploadToTmpfiles } = require('./media');
            let imageUrl = await uploadToTmpfiles(buffer); // tmpfiles is often most reliable
            if (!imageUrl) imageUrl = await uploadToTelegraPh(buffer);
            if (!imageUrl) imageUrl = await uploadToCatbox(buffer);
            
            if (!imageUrl) {
                console.log(chalk.red(`[Vision] All media uploads failed`));
                throw new Error("Upload Failed");
            }
            console.log(chalk.cyan(`[Vision] Uploaded to: ${imageUrl}`));

            // Branch A: Free Gemini (Siputzx)
            try {
                console.log(chalk.blue(`[Vision] Trying Siputzx Gemini...`));
                ...
                console.log(chalk.yellow(`[Vision] Siputzx failed`));
            }

            // Branch B: Free Gemini (Ryzendesu)
            try {
                console.log(chalk.blue(`[Vision] Trying Ryzendesu Gemini...`));
                ...
                console.log(chalk.yellow(`[Vision] Ryzendesu failed`));
            }
            
            throw new Error("All image AI branches failed");
        })());

        // 4. Obito RAW Buffer Vision
        promises.push(getObitoAnalyze(buffer, finalPrompt, mime));

        // Race them exactly like text messages!
        const racePromise = Promise.any(promises.map((p, i) => p.then(res => {
            ...
            // Validate: discard HTML errors or models claiming they can't see the image
            if (lowerRes.includes("<!doctype") || lowerRes.includes("<html")) throw new Error("HTML error");
            if (res.includes("أرسل الصورة") || res.includes("إرسال الصورة") || res.includes("ممكن ترسل لي نصو") || res.includes("أين الصورة") || res.includes("نقدر نشوفه") || lowerRes.includes("can't see")) {
                throw new Error("Model hallucinates about missing image");
            }
            console.log(chalk.green(`[Vision] Provider ${i} succeeded!`));
            return res.trim();
        }).catch(err => {
            console.log(chalk.yellow(`[Vision] Provider ${i} failed: ${err.message}`));
            throw err;
        })));
