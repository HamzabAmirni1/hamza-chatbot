    ...
    try {
        ...
        try {
            ...
            try {
                ...
                // 3. Race 5 free providers in parallel — first non-null wins
                if (!reply) {
                    console.log(chalk.cyan(`[FB AI] Racing free providers...`));
                    reply = await Promise.race([
                        Promise.any([
                            getPollinationsResponse(fbJid, text),
                            getPollinationsQwen(fbJid, text),
                            getAIChartResponse(fbJid, text),
                            getStableAIResponse(fbJid, text),
                            getAutoGPTResponse(fbJid, text),
