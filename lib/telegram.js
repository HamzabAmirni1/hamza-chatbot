    ...
    bot.on('message', async (msg) => {
        ...
        try {
            ...
            try {
                ...
                if (!reply) {
                    console.log(chalk.cyan(`[TG AI] Racing free providers...`));
                    reply = await Promise.race([
                        Promise.any([
                            getAirforceAI(tgJid, text),       // GPT-4o-mini keyless
                            getAkashAI(tgJid, text),          // Llama 3.1 keyless
                            getGithubModelsAI(tgJid, text),   // GPT-4o-mini via GitHub token
                            getPollinationsResponse(tgJid, text), // Pollinations POST
                            getAIChartResponse(tgJid, text),  // Akash fallback
