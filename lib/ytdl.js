...
async function getSiputzx(url, isAudio = false) {
    try {
        const baseURL = 'https://api.siputzx.my.id/api';
        const headers = {
            'authority': 'api.siputzx.my.id',
            'accept': '*/*',
        ...
        for (let i = 0; i < 15; i++) {
            ...
            if (statusResponse.data.status === 'completed') {
                return {
                    download: `https://api.siputzx.my.id${statusResponse.data.download_url}`,
...
async function getVreden(url, isAudio = false) {
    try {
        ...
        const res = await axios.get(`https://api.vreden.my.id/api/${type}?url=${encodeURIComponent(url)}`, { timeout: 15000 });
...
async function getSiputzxY2mate(url, isAudio = false) {
    try {
        const type = isAudio ? 'mp3' : 'mp4';
        const res = await axios.get(`https://api.siputzx.my.id/api/y2mate?url=${encodeURIComponent(url)}&type=${type}`, { timeout: 15000 });
        if (res.data?.status && res.data?.result) {
            return { download: res.data.result, title: 'Siputzx Y2Mate Video' };
        }
        throw new Error('Siputzx Y2Mate failed');
    } catch (e) { throw e; }
}
...
async function downloadYouTube(url, type = 'video') {
    ...
    // Priority Order with more robust fallbacks
    const methods = [
        () => getVreden(url, isAudio),
        () => getSiputzxY2mate(url, isAudio),
        async () => {
        ...
        // Savetube removed due to DNS issues
    ...
    // Extended Fallback APIs
    const fallbackAPIs = [
        {
            name: 'Yupra',
            url: `https://api.yupra.my.id/api/downloader/${isAudio ? 'ytmp3' : 'ytmp4'}?url=${encodeURIComponent(url)}`,
            parser: (data) => data?.data?.download_url ? { download: data.data.download_url, title: data.data.title } : null
        },
        {
            name: 'Okatsu',
            url: `https://okatsu-rolezapiiz.vercel.app/downloader/${isAudio ? 'ytmp3' : 'ytmp4'}?url=${encodeURIComponent(url)}`,
            parser: (data) => data?.result?.[isAudio ? 'mp3' : 'mp4'] ? { download: data.result[isAudio ? 'mp3' : 'mp4'], title: data.result.title } : null
        },
        {
            name: 'Keith',
            url: `https://apis-keith.vercel.app/download/${isAudio ? 'dlmp3' : 'dlmp4'}?url=${encodeURIComponent(url)}`,
            parser: (data) => data?.status && data?.result?.downloadUrl ? { download: data.result.downloadUrl, title: data.result.title } : null
        }
    ];
    ...
    for (const api of fallbackAPIs) {
        try {
            ...
            if (result) {
                console.log(`✅ Fallback ${api.name} successful`);
                return result;
            }
        } catch (e) {
            console.log(`❌ Fallback ${api.name} failed: ${e.message}`);
        }
    }
