...
async function handleIbhaya(platform, senderJid, senderName, text, matchedWord, sockOrBot, msgObj) {
    try {
        ...
        const monitorOnly = cfg.ibhayaMonitorOnly === 'true';
        ...
        // Always log (even in monitor-only mode)
        ...
        logs.unshift({
            ...
            message: text,
            warnings_left: monitorOnly ? (warningsLeft + 1) : warningsLeft,
            timestamp: nowIso
        });
        ...
        try {
            await db.saveDevMessage({
                id: 'ibh_' + Date.now() + '_' + Math.random().toString(36).substr(2, 5),
                sender: senderJid,
                senderName: senderName || 'مستخدم غير معروف',
                platform: platform.toLowerCase(),
                text: `⚠️ [مخالفة إباحية] الكلمة: ${matchedWord}\nالرسالة: ${text}`,
                timestamp: nowIso
            });
        } catch (dbErr) {
            console.error('[Ibhaya] Failed to save violation to DB:', dbErr.message);
        }

        // Restore warnings counter in monitor-only mode (don't consume it)
        if (monitorOnly) {
            await db.setCache(warningsKey, { warnings_left: warningsInfo.warnings_left + 1 });
        }

        console.log(`[Ibhaya] ${monitorOnly ? '[MONITOR]' : ''} Violation logged for ${senderName} (${senderJid}) on ${platform}. Warnings left: ${warningsLeft}`);
        return true;
    } catch (e) {
