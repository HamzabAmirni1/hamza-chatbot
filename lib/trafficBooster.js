...
async function updateProxies() {
    try {
        ...
        // Try multiple sources for US Proxies
        const sources = [
            'https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=5000&country=us&ssl=yes&anonymity=all',
            'https://raw.githubusercontent.com/proxifly/free-proxy-list/main/proxies/countries/US.txt',
            'https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/http.txt' // Global fallback
        ];
        ...
        if (allProxies.length > 0) {
            usProxyList = [...new Set(allProxies)]; // Unique
            console.log(chalk.green(`[Ad-Booster] ✅ Loaded ${usProxyList.length} US/Global proxies.`));
        } else {
            console.log(chalk.red('[Ad-Booster] ⚠️ No proxies found, will retry later.'));
        }
    } catch (e) {
...
async function startTrafficInterval() {
    ...
    // Initial proxy load (Awaited to avoid Direct connection on start)
    await updateProxies();
