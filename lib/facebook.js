...
async function sendFacebookMedia(recipientId, mediaSource, type, caption, pageToken = config.fbPageAccessToken) {
    ...
    // Strategy 3: For audio/video buffers specifically, try uploading to get a URL
    if ((type === 'audio' || type === 'video') && Buffer.isBuffer(mediaSource) && !url) {
        try {
            console.log(chalk.yellow(`[Facebook] Attempting upload to best provider for ${type} buffer...`));
            const ext = type === 'audio' ? 'mp3' : 'mp4';
            const mime = type === 'audio' ? 'audio/mpeg' : 'video/mp4';
            url = await uploadToBestProvider(mediaSource, `media.${ext}`, mime);
            if (url) {
                await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${pageToken}`, {
                    recipient: { id: recipientId },
                    message: { attachment: { type: type, payload: { url: url, is_reusable: true } } }
            ...
            logFacebookError('[Facebook Media Upload Send]', uploadErr, recipientId);
        }
    }
