...
async function simulateGoogleAnalytics(siteUrl, proxyConfig, ua, trafficSource, geo, route = '', eventType = 'page_view', engagementMs = null) {
    ...
    let gaUrl = `https://www.google-analytics.com/g/collect?v=2&tid=${gaId}&cid=${clientId}&en=${eventType}&dl=${encodedUrl}&dr=${encodedRef}&dt=${encodeURIComponent(pageTitle)}&sr=${sr}&ul=${ul}&sid=${sessionId}&sct=1&seg=1`;
...
async function simulateEngagement(siteUrl, proxyConfig, ua, trafficSource, geo) {
    try {
        ...
        // 9. Browse internal pages — realistic multi-page session
        const allRoutes = ['/services', '/portfolio', '/contact', '/projects', '/blog', '/about', '/skills'];
        // Each user visits 1–4 pages (weighted: most visit 1–2)
        ...
        // Simulate realistic section scroll events on homepage first
        const sections = ['hero', 'about', 'services', 'portfolio', 'contact'];
        const scrollDepths = [25, 50, 75, 90];
        for (const depth of scrollDepths) {
            if (Math.random() > 0.3) {
                await new Promise(r => setTimeout(r, Math.random() * 3000 + 1000));
                await simulateGoogleAnalytics(siteUrl, proxyConfig, ua, trafficSource, geo, '', 'scroll', depth * 300);
            }
        }

        // CTA click (30% of users click a button)
        if (Math.random() < 0.3) {
            await simulateGoogleAnalytics(siteUrl, proxyConfig, ua, trafficSource, geo, '', 'click');
        }

        for (const route of pagesToVisit) {
            // Realistic dwell time per page: 12–75 seconds
            const pageTime = Math.floor(Math.random() * 63000) + 12000;
            await new Promise(r => setTimeout(r, Math.random() * 4000 + 2000)); // inter-page nav delay
            ...
            for (const depth of scrollDepths) {
                if (Math.random() > 0.4) {
                    ...
                    await simulateGoogleAnalytics(siteUrl, proxyConfig, ua, trafficSource, geo, route, 'scroll', depth * 300);
                }
            }

            // Section view events (unique to each page)
            if (Math.random() > 0.5) {
                await simulateGoogleAnalytics(siteUrl, proxyConfig, ua, trafficSource, geo, route, 'view_item');
            }
            // Contact/CTA click (higher on contact page)
            const clickChance = route === '/contact' ? 0.55 : route === '/portfolio' ? 0.4 : 0.25;
