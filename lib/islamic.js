...
async function startDuasScheduler(sock, helpers) {
    const { sendWithChannelButton, config } = helpers;

    console.log('[Duas] 🤲 Initializing duas scheduler data from Supabase...');
    try {
        const remoteDuas = await db.loadDuasData();
        if (remoteDuas) {
            const dataDir = path.dirname(DUAS_PATH);
            fs.ensureDirSync(dataDir);
            fs.writeFileSync(DUAS_PATH, JSON.stringify(remoteDuas, null, 2));
            console.log('[Duas] 🤲 Synced duas subscriptions from Supabase.');
        }
    } catch (e) {
        console.error('[Duas] 🤲 Failed to sync duas data from Supabase:', e.message);
    }

    setInterval(async () => {
        try {
            const data = loadDuasData();
            if (!data.enabled || config.enableDuasScheduler === 'false') return;
            ...
            const targetHours = Array.isArray(config.duasHours) ? config.duasHours : [7, 9, 11, 12, 17, 19, 22];
