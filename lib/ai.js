...
async function analyzeImage(buffer, mime, prompt) {
    try {
        ...
        // 3. Obito Vision API & Free Gemini API (Upload required)
        promises.push((async () => {
            const { uploadToCatbox, uploadToTelegraPh } = require('./media');
            let imageUrl = await uploadToTelegraPh(buffer);
            ...
            // Branch C: Pollinations AI
            const { data } = await axios.post("https://text.pollinations.ai/", {
                model: "openai",
                messages: [
                    { role: "system", content: "أنت مساعد ذكي متعدد اللغات (دارجة، فرنسية، إنجليزية). أجب مباشرة على سؤال المستخدم حول الصورة بأسلوب طبيعي." },
                    { role: "user", content: [
