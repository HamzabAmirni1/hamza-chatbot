...
function saveState(state) {
    ...
    db.savePrayerState(state).catch(e => {});
}
...
function saveSubs(platform, subs) {
    ...
    db.savePrayerSubs(platform, subs).catch(e => {});
}
...
async function startPrayerScheduler(sock) {
    ...
    console.log('[Prayer] 🕌 Initializing prayer scheduler data from Supabase...');
    try {
        const remoteState = await db.loadPrayerState();
        if (remoteState) {
            fs.ensureDirSync(DATA_DIR);
            fs.writeFileSync(PRAYER_STATE_FILE, JSON.stringify(remoteState, null, 2));
        }
        for (const platform of ['wa', 'tg', 'fb']) {
            const remoteSubs = await db.loadPrayerSubs(platform);
            if (remoteSubs) {
                const filePath = getSubsFile(platform);
                fs.ensureDirSync(DATA_DIR);
                fs.writeFileSync(filePath, JSON.stringify(remoteSubs, null, 2));
            }
        }
        console.log('[Prayer] 🕌 Synced all prayer subscriptions and state from Supabase.');
    } catch (e) {
        console.error('[Prayer] 🕌 Failed to sync prayer data from Supabase:', e.message);
    }
    ...
    global.prayerInterval = setInterval(async () => {
        try {
            ...
            const state = readState();
            if (!state.enabled || config.enablePrayerScheduler === 'false') return;

            const now = moment().tz('Africa/Casablanca');
            const todayKey = now.format('YYYY-MM-DD');
            const currentHHMM = now.format('HH:mm');

            // ALL platforms are opt-in: only subscribed users receive reminders.
            // User subscribes by typing: .salat on  OR  .salat [city name]
            const platforms = ['wa', 'tg', 'fb'];

            for (const platId of platforms) {
                const subsMap = readSubs(platId);
                const subscribedUsers = Object.keys(subsMap); // only subscribed users

                for (const userId of subscribedUsers) {
                    const pref = subsMap[userId];
                    const timings = await fetchPrayerTimes(pref.city || state.city, pref.country || state.country, state.method);
                    if (!timings) continue;
                    ...
                    for (const prayer of prayers) {
                        ...
                        if (currentHHMM === pTime && !pref.lastSent[sentId]) {
                            ...
                            changed = true;
                            const message = buildPrayerMessage(prayer, pTime, pref.city || state.city);

                            // Send based on platform
                            try {
                                if (platId === 'wa') {
                                    await sendWithChannelButton(currentSock, userId, message);
                                } else if (platId === 'tg' && config.telegramToken) {
                                    ...
                                    // pageId stored in pref if set
                                    await sendFacebookMessage(userId, message.replace(/\*/g, '').replace(/_/g, ''), pref.pageId || null);
                                }
                                console.log(`[Prayer] Sent ${prayer} to ${platId} user ${userId} in ${pref.city || state.city}`);
                            } catch (e) {
                                console.error(`[Prayer] Failed to send ${prayer} to ${platId} user ${userId}:`, e.message);
                            }
                        }
                    }

                    // Cleanup lastSent for old days and persist if changed
                    for (const key of Object.keys(pref.lastSent)) {
                        if (!key.startsWith(todayKey)) delete pref.lastSent[key];
                    }
                    if (changed) {
                        subsMap[userId] = pref;
                        saveSubs(platId, subsMap);
                    }
                }
            }
