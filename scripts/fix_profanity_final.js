...
// We write clean JS using template literals carefully
// Key: inside template literals for HTML, we use \" for HTML attrs to avoid JS quote conflict
// We avoid embedding JS expressions that have single-quotes inside single-quoted strings
const cleanFn = `  async function loadProfanityLogs() {
    ...
    try {
      ...
      const platformIcon  = { WA: '\u{1F7E2}', TG: '\u2708\uFE0F', FB: '\u{1F535}' };
      ...
      const rowsHtml = users.map(u => {
        ...
        const wLabel = wLeft <= 0 ? '\u{1F6AB} \u0645\u062d\u0638\u0648\u0631' : \`\${wLeft} / 3\`;
        const ts     = new Date(u.timestamp).toLocaleString('ar-MA');
        const pkey   = u.platform;
        const sName  = escapeHtml(u.name || '\u0645\u0633\u062a\u062e\u062f\u0645');
        return \`<tr style="border-bottom:1px solid var(--border);">
          ...
          <td style="padding:10px 12px;"><code style="background:rgba(252,129,129,0.12);color:#fc8181;padding:2px 7px;border-radius:4px;">\${escapeHtml(u.bad_word||'')}</code></td>
          ...
          <td style="padding:10px 12px;">
            <div style="display:flex;gap:5px;flex-wrap:wrap;">
              <button onclick="openProfanityMsgModal('\${u.jid}','\${pkey}','\${(u.name||'').replace(/'/g,String.fromCharCode(92,39))}')" style="padding:4px 9px;font-size:11px;border:1px solid rgba(99,179,237,0.4);background:rgba(99,179,237,0.08);color:#63b3ed;border-radius:6px;cursor:pointer;font-family:Cairo,sans-serif;white-space:nowrap;" title="\u0625\u0631\u0633\u0627\u0644 \u0631\u0633\u0627\u0644\u0629"><i class="fas fa-paper-plane"></i> \u0631\u0633\u0627\u0644\u0629</button>
              <button onclick="resetProfanityWarnings('\${u.jid}')" style="padding:4px 9px;font-size:11px;border:1px solid rgba(104,211,145,0.4);background:rgba(104,211,145,0.08);color:#68d391;border-radius:6px;cursor:pointer;font-family:Cairo,sans-serif;white-space:nowrap;"><i class="fas fa-redo"></i> \u0625\u0639\u0627\u062f\u0629 \u0636\u0628\u0637</button>
              <button onclick="banProfanityUser('\${u.jid}')" style="padding:4px 9px;font-size:11px;border:1px solid rgba(252,129,129,0.4);background:rgba(252,129,129,0.08);color:#fc8181;border-radius:6px;cursor:pointer;font-family:Cairo,sans-serif;white-space:nowrap;"><i class="fas fa-ban"></i> \u062d\u0638\u0631</button>
              <button onclick="unbanProfanityUser('\${u.jid}')" style="padding:4px 9px;font-size:11px;border:1px solid rgba(246,224,94,0.4);background:rgba(246,224,94,0.08);color:#f6e05e;border-radius:6px;cursor:pointer;font-family:Cairo,sans-serif;white-space:nowrap;"><i class="fas fa-unlock"></i> \u0631\u0641\u0639 \u062d\u0638\u0631</button>
            </div>
          </td>
        </tr>\`;
      }).join('');
...
const newContent = before + cleanFn + after;

fs.writeFileSync(filePath, newContent, 'utf8');
console.log('Done! Size:', Buffer.byteLength(newContent, 'utf8'));
