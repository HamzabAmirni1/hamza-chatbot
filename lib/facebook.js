    ...
    try {
        ...
        if (senderId && pageToken) {
            ...
            if (!global.fbNames[cleanSenderId]) {
                fetchFbProfileName(cleanSenderId, pageToken).then(name => {
                    if (name) {
                        global.fbNames[cleanSenderId] = name;
                        db.saveUserNames('facebook', global.fbNames).catch(() => {});
        ...
        try {
            ...
            try {
                ...
                reply = await getStableAIResponse(senderId, text) || await getBlackboxResponse(senderId, text);
                if (reply) {
