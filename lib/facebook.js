...
async function handleFacebookMessage(event) {
    try {
        ...
        const isNewUser = saveFbUser(senderId, pageId);
        
        if (isNewUser) {
            const welcomeMsg = `👋 *مرحباً بك في بوت حمزة اعمرني!* ✨\n\n` +
                             `شكراً لتواصلك معنا. للاستفادة من كافة ميزات البوت والتوصل بجديدنا، ندعوك لـ:\n\n` +
                             `📸 *متابعة حسابنا على إنستغرام:* \n${config.instagram || 'https://instagram.com/HamzabAmirni1'}\n\n` +
                             `✅ *متابعة قناتنا على واتساب:* \n${config.officialChannel}\n\n` +
                             `👍 *الإعجاب بصفحتنا على فيسبوك:* \n${config.facebookPage}\n\n` +
                             `🤖 يمكنك الآن البدء في الدردشة أو إرسال صور لتحليلها!`;
            await sendFacebookMessage(senderId, welcomeMsg, pageToken);
        }
                    ...
                    if (cmdMatch) {
                        ...
                        if (allCmds[command]) {
                            try {
                                ...
                                const mockSock = createMockSock(senderId, mediaUrl, pageToken);
                                const msg = { key: { remoteJid: senderId, fromMe: false, id: Date.now().toString() }, pushName: "FB User", body: text };
                                await cmdFile(mockSock, senderId, msg, args, { isFacebook: true, command: command, buffer: null }, "ar");
