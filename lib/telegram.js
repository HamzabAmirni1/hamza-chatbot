                    ...
                    try {
                        ...
                        const mockSock = {
                            sendMessage: async (id, content, opts) => {
                                ...
                                if (content.image) {
                                    try {
                                        ...
                                        if (typeof photoSource === 'string' && photoSource.startsWith('http')) {
                                            return await bot.sendPhoto(id, photoSource, { caption: content.caption });
                                        } else {
                                            return await bot.sendPhoto(id, photoSource, { caption: content.caption });
                                        }
                                    } catch (e) {
                                        if (content.image.url) {
                                            try {
                                                ...
                                                return await bot.sendPhoto(id, Buffer.from(resp.data), { caption: content.caption });
                                            } catch (err) {
                                                return bot.sendMessage(id, "⚠️ فشل إرسال الصورة.");
                                            }
                                        }
                                    }
                                }
                                if (content.video) {
                                    try {
                                        return await bot.sendVideo(id, content.video.url, { caption: content.caption });
                                    } catch (e) {
                                        return bot.sendMessage(id, "⚠️ فشل إرسال الفيديو.");
                                    }
                                }
                        ...
                        try {
                            ...
                            const mockSock = {
                                sendMessage: async (id, content, opts) => {
                                    ...
                                    if (content.image) {
                                        try {
                                            return await bot.sendPhoto(id, content.image.url, { caption: content.caption });
                                        } catch (e) {
                                            const resp = await axios.get(content.image.url, { responseType: 'arraybuffer' });
                                            return await bot.sendPhoto(id, Buffer.from(resp.data), { caption: content.caption });
                                        }
