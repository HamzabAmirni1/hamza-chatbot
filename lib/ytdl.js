...
async function getBTCH(url, isAudio = false) {
    try {
        ...
        if (res.data?.status && res.data.result?.url) {
            return {
                download: res.data.result.url,
                title: res.data.result.title || 'BTCH Download',
                thumb: res.data.result.thumbnail
            };
        }
...
async function getShizuka(url, isAudio = false) {
    try {
        ...
        if (res.data?.status && res.data.result?.downloadUrl) {
            return {
                download: res.data.result.downloadUrl,
                title: res.data.result.title || 'Shizuka Download'
            };
        }
        throw new Error('Shizuka failed');
    } catch (e) { throw e; }
}
    ...
    for (const method of methods) {
        try {
            ...
            if (result && result.download) {
                // Validate if the URL is actually reachable (404/403 check)
                const isValid = await checkLink(result.download, result.referer);
                if (isValid) {
                    console.log(`✅ Download successful via ${method.name}`);
                    return result;
                } else {
                    console.log(`⚠️ ${method.name} returned an invalid or blocked URL (404/403)`);
                }
            }
        } catch (e) {
...
async function checkLink(url, referer = null) {
    try {
        ...
        const res = await axiosIgnoreSSL.get(url, {
            headers,
            timeout: 5000,
            maxContentLength: 1, // We only need headers/status
            validateStatus: (status) => status >= 200 && status < 400
        });
        return true;
    } catch (e) {
