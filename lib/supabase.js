...
const db = {
    ...
    async hasUser(jid) {
        if (!supabase) return false;
        try {
            const { data } = await supabase.from('ai_memory').select('jid').eq('jid', jid).single();
            return !!data;
        } catch (e) {
            return false;
        }
    ...
    async deleteErrorLog(id) {
        ...
        try {
            ...
            console.error('[Supabase] deleteErrorLog Error:', e.message);
            return false;
        }
    },
    ...
    async clearAllErrorLogs() {
        ...
        try {
            ...
            const { error } = await supabase.from('error_logs').delete().neq('id', -1);
            if (error) throw error;
            return true;
        } catch (e) {
            console.error('[Supabase] clearAllErrorLogs Error:', e.message);
            return false;
        }
    },
    ...
    async saveDevMessage({ id, sender, senderName, platform, text, timestamp, pageId }) {
        if (!supabase) return false;
        try {
            const parts = (platform || '').split(':');
            const cleanPlatform = parts[0] || 'whatsapp';
            const resolvedPageId = pageId || parts[1] || null;
            const finalPlatform = cleanPlatform + (resolvedPageId ? ':' + resolvedPageId : '');
            ...
            const { error } = await supabase.from('dev_messages').insert(insertData);
            if (error) throw error;
            return true;
        } catch (e) {
    ...
    async saveDevReply({ id, sender, senderName, platform, replyText, timestamp, pageId }) {
        if (!supabase) return false;
        try {
            const parts = (platform || '').split(':');
            const cleanPlatform = parts[0] || 'whatsapp';
    ...
    async getDevMessages(limit = 500) {
        ...
        try {
            ...
            // and parse pageId from platform field if present
            return (data || []).map(m => {
                const platformParts = (m.platform || '').split(':');
                const cleanPlatform = platformParts[0] || 'whatsapp';
                const pageId = platformParts[1] || null;
                return {
                    id: m.id,
                    sender: m.sender,
    ...
    async markAllDevMessagesFromSenderReplied(sender) {
        if (!supabase) return false;
        try {
            const { error } = await supabase.from('dev_messages').update({
                replied: true,
                reply_timestamp: new Date().toISOString()
            }).eq('sender', sender).eq('replied', false);
            if (error) throw error;
            return true;
        } catch (e) {
            console.error('[Supabase] markAllDevMessagesFromSenderReplied Error:', e.message);
            return false;
        }
    },

    async deleteDevMessage(idOrTimestamp) {
        if (!supabase) return false;
        try {
            const { error } = await supabase
                .from('dev_messages')
                .delete()
                .or(`id.eq.${idOrTimestamp},timestamp.eq.${idOrTimestamp}`);
            if (error) throw error;
            return true;
        } catch (e) {
