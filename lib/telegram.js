    ...
    function createMockSock(bot, msg, chatId) {
        const mockSock = {
            ...
            sendMessage: async (id, content, opts) => {
                ...
                if (content.video) return bot.sendVideo(id, content.video.url || content.video, { caption: content.caption, ...options });
                if (content.audio) return bot.sendAudio(id, content.audio.url || content.audio, { caption: content.caption, ...options });
