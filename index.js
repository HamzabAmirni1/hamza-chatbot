...
🤖 *نماذج الدردشة (AI CHAT):*
├ 🤖 *.gpt4o* ┈ GPT-4o
├ ⚡ *.gpt4om* ┈ 4o Mini
├ 🧠 *.o1* ┈ OpenAI O1
└ 💬 *Auto-Reply*

📡 *خدمات إضافية (SERVICES):*
├ 📱 *.tempnum* ┈ أرقام وهمية
├ 🔍 *.yts* ┈ بحث يوتيوب
└ 🏓 *.ping* ┈ سرعة البوت
...
📌 *Uptime:* ${getUptime()}`;
                    ...
                    const buttons = [
                        { buttonId: '.tempnum', buttonText: { displayText: '📱 أرقام وهمية' }, type: 1 },
                        { buttonId: '.yts quran', buttonText: { displayText: '🔍 بحث يوتيوب' }, type: 1 },
                        { buttonId: '.ping', buttonText: { displayText: '🏓 سرعة البوت' }, type: 1 }
                    ];

                    const imagePath = path.join(__dirname, 'media', 'hamza.jpg');
                    const message = {
                        image: fs.existsSync(imagePath) ? { url: imagePath } : { url: 'https://pollinations.ai/p/cool-robot-assistant' },
                        caption: menuText,
                        footer: `⚔️ ${config.botName} • Active 24/7 on Koyeb`,
                        buttons: buttons,
                        headerType: 4,
                        viewOnce: true // Force viewOnce for button visibility
                    };

                    await sock.sendMessage(sender, message, { quoted: msg });
                    await sock.sendMessage(sender, { react: { text: "📜", key: msg.key } });
                    continue;
                }
