...
async function getHFVision(imageBuffer, prompt = "Explain everything in this image in detail, including any text you see (OCR).") {
    try {
...
async function getObitoAnalyze(imageBuffer, prompt = "ما الموجود في هذه الصورة؟ قم بوصفها بدقة للمستخدم مع قراءة أي نصوص واضحة.", mime = "image/jpeg") {
...
✨ *Active 24/7 on Koyeb* ✨`;
                ...
                if (body && body.match(/^\.(weather|حالة-الطقس|طقس|جو)\s*(.*)/i)) {
                    ...
                    if (!city) {
                        await sendWithChannelButton(sock, sender, `🌍 *حالة الطقس (Weather)*\n\n📝 *الطريقة:* .weather [اسم المدينة]\n*مثال:* .weather Casablanca\n\n⚔️ ${config.botName}`, msg);
                        continue;
                    }
                    ...
                    try {
                        ...
                        if (!data.success || !data.data) {
                            await sendWithChannelButton(sock, sender, `❌ ما لقيتش معلومات على المدينة: *${city}*`, msg);
                            continue;
                        }

                        const d = data.data;
                        const emoji = getWeatherEmoji(d.weather);
                        const weatherText = `🌍 *حالة الطقس في ${d.location}, ${d.country}*\n\n` +
                        ...
                        await sendWithChannelButton(sock, sender, `❌ وقع مشكل فجلب حالة الطقس. جرب من بعد.`, msg);
                    }
                    continue;
                }
                ...
                // 🏓 PING COMMAND
                if (body && (body.toLowerCase() === '.ping' || body.toLowerCase() === 'ping' || body.toLowerCase() === 'بينج')) {
                    const start = Date.now();
                    await sock.sendMessage(sender, {
                ...
                if (type === 'imageMessage' || type === 'videoMessage') {
                    ...
                    try {
                        ...
                        if (isVideo) {
                            ...
                            if (config.geminiApiKey) {
                                ...
                                if (reply) console.log(chalk.green("✅ Gemini Vision responded."));
                            }

                            if (!reply && config.openRouterKey) {
                                reply = await getOpenRouterResponse(sender, prompt, buffer);
                                if (reply) console.log(chalk.green("✅ OpenRouter Vision responded."));
                            }
                    ...
                    const isRecentImg = lastImg && (Date.now() - lastImg.timestamp < 5 * 60 * 1000); // 5 minutes

                    if (isRecentImg && body.length > 2 && !body.startsWith('.')) {
                        console.log(chalk.cyan("🔍 Detected recent image. Checking with Vision models..."));

                        // Try Gemini first (Best for context)
                        if (config.geminiApiKey) {
                            reply = await getGeminiResponse(sender, body, lastImg.buffer, lastImg.mime);
                        }
                        ...
                        if (reply) console.log(chalk.green("✅ Image-Context reply generated."));
                    }
