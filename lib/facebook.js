...
const { getContext, addToHistory, getAutoGPTResponse, getHectormanuelAI, getGeminiResponse, getLuminAIResponse, getAIDEVResponse, getPollinationsResponse, getBlackboxResponse, getStableAIResponse, getOpenRouterResponse, detectLanguage, webSearch, needsWebSearch, getDuckDuckGoAI, getTypeGPTResponse, getNerimityAI, getPollinationsQwen, getAIChartResponse, getAirforceAI, getAkashAI, getGithubModelsAI, getChatUpAI, getNowTechAI, getChatEverywhereAI } = require('./ai');
const chalk = require('chalk');
const fs = require('fs-extra');
const path = require('path');
const FormData = require('form-data');
const { ALL_COMMANDS, NLC_KEYWORDS, isQuestionOrInquiry, handleAutoDownload } = require('./commandMap');
const { checkSubscriptionGate, getSubscriptionMessage, getWelcomeMessage, autoSubscribeUser } = require('./subscription');
...
// Returns true if this is a NEW user (first message ever)
async function saveFbUser(senderId, pageId) {
    try {
        ...
        // If not found in local JSON, check if they exist in Supabase DB
        const existsInDb = await db.hasUser(`fb:${id}`);
        ...
        if (!existsInDb) {
            ...
            // ── AUTO-SUBSCRIBE to prayer reminders (opt-out model) ──
            try {
                const { subscribeUser, isSubscribed } = require('./prayerScheduler');
                if (!isSubscribed(id, 'fb')) {
                    subscribeUser(id, config.defaultPrayerCity || 'Casablanca', 'MA', 'fb');
                    // Patch pageId into prayer subs
                    const prayerSubsPath = path.join(__dirname, '..', 'data', 'fb_prayer_subs.json');
                    if (fs.existsSync(prayerSubsPath)) {
                        const subs = JSON.parse(fs.readFileSync(prayerSubsPath, 'utf8') || '{}');
                        if (subs[id]) { subs[id].pageId = pId; fs.writeFileSync(prayerSubsPath, JSON.stringify(subs, null, 2)); }
                    }
                    console.log(`[Prayer] Auto-subscribed new FB user ${id} to prayer reminders (Casablanca)`);
                }
            } catch (e) {}

            return true; // Brand new user (welcome message will be sent)
        } else {
            return false; // Existing user in Supabase DB (no welcome message)
        }
...
async function sendFacebookMessage(recipientId, text, pageTokenOrId = config.fbPageAccessToken, tag = null) {
    ...
    try {
        ...
        for (const chunk of chunks) {
            ...
            // Strategy: Try HUMAN_AGENT tag first (7-day window, requires feature approval).
            // If rejected with error_subcode 1893010 (feature not enabled) or 200 (permission),
            // fallback to standard RESPONSE type (24h window).
            const strategies = [
                // 1st attempt: HUMAN_AGENT — works up to 7 days after last user message
                {
                    messaging_type: 'MESSAGE_TAG',
                    tag: 'HUMAN_AGENT',
                    message: { text: chunk }
                },
                // 2nd attempt: standard RESPONSE — works within 24h window
                {
            ...
            // If an explicit tag was passed, use it as first strategy
            if (tag) {
                strategies.unshift({
                    messaging_type: 'MESSAGE_TAG',
                    tag,
            ...
            for (const payloadBase of strategies) {
                try {
                    const payload = {
                    ...
                    chunkSent = true;
                    if (payloadBase.tag === 'HUMAN_AGENT') {
                        console.log(chalk.cyan(`[Facebook] ✅ Sent via HUMAN_AGENT tag (7-day window) to ${recipientId}`));
                    }
                    break;
                } catch (stratErr) {
                    const errCode = stratErr?.response?.data?.error?.code;
                    const subCode = stratErr?.response?.data?.error?.error_subcode;
                    // Only fallback if FB rejected for policy/permission reasons
                    // 2018276 = HUMAN_AGENT tag not approved for this page
                    const isPermissionError = [100, 200, 10, 613].includes(errCode) || [1893010, 1893061, 2018001, 2018144, 2018276].includes(subCode);
                    if (isPermissionError) {
                        console.warn(chalk.yellow(`[Facebook] Strategy (${payloadBase.tag || payloadBase.messaging_type}) rejected (${errCode}/${subCode}), trying next...`));
                        lastChunkError = stratErr;
                        continue;
                    }
                    // Non-policy error → propagate immediately
                    throw stratErr;
                }
            }
            if (!chunkSent) throw lastChunkError;
    ...
    try {
        ...
        try {
            if (global.bannedUsersCache && (global.bannedUsersCache.includes(`fb:${senderId}`) || global.bannedUsersCache.includes(senderId.toString()))) {
                const fbMsgText = event.message?.text || "";
                const isMsgToDev = fbMsgText && (fbMsgText.trim().startsWith('.msgtodev') || fbMsgText.trim().startsWith('!msgtodev'));
                if (!isMsgToDev) {
        ...
        // ── Handle Quick Reply button payloads (prayer reminder buttons) ──
        const QUICK_REPLY_MAP = {
            'CMD_SALAT_NOW': '.salat now',
            'CMD_SALAT_CITY': '.salat',
            'CMD_SALAT_OFF': '.salat off',
            'CMD_HELP': '.help'
        };
        if (message.quick_reply?.payload && QUICK_REPLY_MAP[message.quick_reply.payload]) {
            message.text = QUICK_REPLY_MAP[message.quick_reply.payload];
        }
        ...
        const isMsgToDev = text && (text.trim().startsWith('.msgtodev') || text.trim().startsWith('!msgtodev'));
        if (!isMsgToDev) {
            const gateStatus = checkSubscriptionGate(senderId, 'fb');
            if (gateStatus === 'blocked') {
                await sendFacebookMessage(senderId, getSubscriptionMessage('fb'), pageToken);
                return;
            } else if (gateStatus === 'pending') {
                await sendFacebookMessage(senderId, getWelcomeMessage(), pageToken);
                // Let the first command run as normal
            }
        }
        // Auto-subscribe to daily reminders & prayers
        autoSubscribeUser(senderId, 'fb');
        // ========================================================

        const isNewUser = await saveFbUser(senderId, pageId); // save for broadcast + auto-subscribe prayer

        // ── Welcome message for NEW users with prayer reminder info + quick-reply buttons ──
        if (isNewUser) {
            try {
                const { getUserCity } = require('./prayerScheduler');
                const defaultCity = getUserCity(senderId, 'fb') || 'Casablanca';
                const welcomeText =
                    `🌙 أهلاً وسهلاً بك في *${config.botName}*! 🎉\n\n` +
                    `━━━━━━━━━━━━━━━━━━\n` +
                    `🕌 *تم تفعيل تذكير أوقات الصلاة تلقائياً!*\n` +
                    `📍 مدينتك الافتراضية: *${defaultCity}*\n\n` +
                    `يمكنك تغيير مدينتك في أي وقت بكتابة:\n` +
                    `📝 *.salat [اسم المدينة]* — مثال: *.salat Marrakech*\n` +
                    `📝 *.salat off* — لإيقاف التذكير\n` +
                    `━━━━━━━━━━━━━━━━━━\n` +
                ...
                const quickReplyPayload = {
                    ...
                    message: {
                        ...
                        quick_replies: [
                            { content_type: 'text', title: '🕌 أوقات الصلاة', payload: 'CMD_SALAT_NOW' },
                            { content_type: 'text', title: '🌍 تغيير مدينتي', payload: 'CMD_SALAT_CITY' },
                            { content_type: 'text', title: '🔕 إيقاف التذكير', payload: 'CMD_SALAT_OFF' },
                            { content_type: 'text', title: '📋 قائمة الأوامر', payload: 'CMD_HELP' }
                        ]
                    }
                };
                await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${pageToken}`, quickReplyPayload);
            } catch (e) {
                console.error('[Prayer Welcome] Failed to send welcome message:', e.message);
            }
        }
        ...
        try {
            ...
            if (cmdMatch) {
                ...
                if (allCmds[command]) {
                    ...
                    try {
                        await cmdFile(mockSock, senderId, msg, args, { isFacebook: true, command: command, buffer: cmdBuffer, pageId: pageId }, detectLanguage(text));
                    } catch (cmdErr) {
                ...
                for (const [key, path] of Object.entries(nlcKeywords)) {
                    ...
                    if (new RegExp(`(^|\\s)(${key})(\\s|$)`, "i").test(lowerBody)) {
                        try {
                            ...
                            await cmdFile(mockSock, senderId, msg, rest, { isFacebook: true, command: key.split("|")[0], buffer: nlcBuffer, pageId: pageId }, detectLanguage(text));
                            commandHandled = true;
            ...
            // Follow-up on recent image — only if the user is actually asking about the image
            const context = await getContext('fb:' + senderId);
            const isRecentImg = context.lastImage && Date.now() - context.lastImage.timestamp < 5 * 60 * 1000;
            const imageFollowUpKeywords = /حلل|تحليل|شنو|اشرح|وصف|معنى|فيها|فيه|هذي|هاد|صور|analyze|describe|explain|what|شنو في|اش في|شو في|شبها|كيفاش|كيف|اشبه/i;
            if (isRecentImg && text.length > 2 && !text.startsWith(".") && imageFollowUpKeywords.test(text)) {
                try {
            ...
            try {
                ...
                if (config.geminiApiKey) {
                    console.log(chalk.cyan(`[FB AI] Trying Gemini...`));
                    reply = await getGeminiResponse(fbJid, text);
                }

                // 2. Try OpenRouter if configured
                if (!reply && config.openRouterKey) {
                    console.log(chalk.cyan(`[FB AI] Trying OpenRouter...`));
                    reply = await getOpenRouterResponse(fbJid, text);
                }

                // 3. Try ChatEverywhere (GPT-4) first — best Arabic & Darija understanding
                if (!reply) {
                    console.log(chalk.cyan(`[FB AI] Trying ChatEverywhere (GPT-4) — best Arabic/Darija...`));
                    reply = await Promise.race([
                        getChatEverywhereAI(fbJid, text),
                        new Promise((_, rej) => setTimeout(() => rej(), 8000))
                    ]).catch(() => null);
                }

                // 4. Race free providers in parallel — first non-null wins
                if (!reply) {
                    console.log(chalk.cyan(`[FB AI] Racing Akash and Silana models...`));
                    reply = await Promise.race([
                        Promise.any([
                            getAkashAI(fbJid, text),             // 4 Akash models in parallel
                            getChatUpAI(fbJid, text),            // ChatUpAI keyless
                            getNowTechAI(fbJid, text),           // NowTechAI keyless
                            getPollinationsResponse(fbJid, text), // Pollinations POST fallback
                        ].map(p => p.then(r => { if (!r) throw new Error('empty'); return r; }))),
                        new Promise((_, rej) => setTimeout(() => rej(), 18000))
                    ]).catch(() => null);
                }
            } catch (e) {
                console.error(`[FB AI] Error in sequential flow: ${e.message}`);
            }
                    ...
                    if (cmdMatch) {
                        ...
                        if (allCmds[command]) {
                            try {
                                ...
                                const mockSock = createMockSock(senderId, mediaUrl, pageToken);
                                const msg = { key: { remoteJid: senderId, fromMe: false, id: Date.now().toString() }, pushName: displayName, body: text };
                                await cmdFile(mockSock, senderId, msg, args, { isFacebook: true, command: command, buffer: null, pageId: pageId }, detectLanguage(text));
