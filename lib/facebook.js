    ...
    try {
        ...
        try {
            ...
            try {
                ...
                if (!reply) {
                    console.log(chalk.cyan(`[FB AI] Racing free providers...`));
                    reply = await Promise.race([
                        Promise.any([
                            getAirforceAI(fbJid, text),       // GPT-4o-mini keyless
                            getAkashAI(fbJid, text),          // Llama 3.1 keyless
                            getGithubModelsAI(fbJid, text),   // GPT-4o-mini via GitHub token
                            getPollinationsResponse(fbJid, text), // Pollinations POST
                            getAIChartResponse(fbJid, text),  // Akash fallback
