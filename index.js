                ...
                if (body && body.match(/^\.(yts|بحث-يوتيوب|chercher)\s+(.+)/i)) {
                    ...
                    try {
                        ...
                        const buttons = videos.map((v, i) => {
                            ...
                            resultText += `   ⏱️ ${v.timestamp} • 👁️ ${v.views.toLocaleString()}\n`;
                            resultText += `   📅 ${v.ago}\n\n`;

                            return {
                                buttonId: `.video ${v.url}`,
                                buttonText: { displayText: `📥 ${i + 1}. ${v.title.substring(0, 20)}...` },
                                type: 1
                            };
                        });

                        resultText += `\n💡 *اختر رقم الفيديو اللي بغيتي تحملو*\n⚔️ *${config.botName}*`;
