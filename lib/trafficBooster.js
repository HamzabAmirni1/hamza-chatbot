...
async function boostTraffic() {
    ...
    for (const url of urlsToBoost) {
        try {
            ...
            visitCounter++;
            console.log(chalk.blue(`[Ad-Booster] 🚀 Target hit (US Proxy): ${url} | Total: ${visitCounter}`));
...
async function startTrafficInterval() {
    console.log(chalk.green('🔥 Mega Ad-Revenue & Traffic Booster Active (v3.0). Target: 1000+ Daily Safe Visits.'));

    await updateProxies();
    setInterval(updateProxies, 30 * 60 * 1000);

    // Summary log every 1 hour
    setInterval(() => {
        console.log(chalk.magenta(`\n📊 [Ad-Booster Summary] Total successful visits since bot start: ${visitCounter}\n`));
    }, 60 * 60 * 1000);

    // Parallelism: Simulate 3-5 users at once to reach 1000+ comfortably
    const workerCount = 4;
    ...
    for (let i = 0; i < workerCount; i++) {
        const run = async () => {
            try {
                ...
                // If you have a Monetag Direct Link, add it to config.js to boost it specifically
                if (config.monetagDirectLink) {
                    await boostTraffic(config.monetagDirectLink);
                }
            } catch (e) { }

            // To reach 1000 visitors: 1440 minutes / (1000/workerCount) = ~5-6 mins per visit
            // Randomized delay between 3 to 8 minutes per worker thread
            const nextRun = (3 + Math.random() * 5) * 60 * 1000;
            setTimeout(run, nextRun);
        };

        // Stagger worker starts
        setTimeout(run, i * 20000);
    }
}
