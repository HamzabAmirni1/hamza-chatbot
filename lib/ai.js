...
function needsWebSearch(text) {
    ...
    const clean = text.trim().toLowerCase()
        ...
        .replace(/^(يا\s+|يا\s*بوت\s+|البوت\s+)/i, ''); // remove calling prefixes

    // Simple common greetings & casual short phrases to answer instantly
    const simpleChats = [
        'سلام', 'السلام', 'مرحبا', 'مرحب', 'اهلين', 'أهلاً', 'اهلاً', 'هلو',
        'كي داير', 'كيداير', 'لاباس', 'لا باس', 'بخير', 'صباح الخير', 'مساء الخير',
        'hi', 'hello', 'hey', 'yo', 'hola', 'bonjour', 'salam', 'fine', 'how are you',
        'شكون أنت', 'شكون انت', 'من أنت', 'من انت', 'من المطور', 'شكون مطور',
        'أوامر', 'اوامر', 'مساعدة', 'المساعدة', 'help', 'menu', 'شكون حمزة', 'من هو حمزة'
    ];
    
    // Check if the query is a simple greeting or casual chat
    const isGreeting = simpleChats.some(c => clean === c || clean.startsWith(c + ' ') || clean.endsWith(' ' + c));
    if (isGreeting) {
        return false;
    }
    
    // Enable Web Search by default for any actual questions/topics
    return true;
}
...
function sanitizeAiResponse(text) {
    ...
    // Fix Broken Markdown Links on WhatsApp (e.g. [https://www](https://...) -> https://...)
    clean = clean.replace(/\[([^\]]*)\]\((https?:\/\/[^\s\)]+)\)/gi, (match, label, url) => {
        if (label.startsWith('http') || label === url) return url;
        return `${label}: ${url}`;
    });
...
async function getPollinationsResponse(jid, message) {
    // Try POST first (with history context)
    try {
        const history = (await getContext(jid)).messages.slice(-4); // limit history to avoid large payloads
        const messages = [
        ...
        const { data } = await axios.post("https://text.pollinations.ai/", {
            ...
            headers: {
                'Content-Type': 'application/json',
                'User-Agent': `Mozilla/5.0 (compatible; HamzaBot/${Math.floor(Math.random()*99)+1}.0)`
            }
        });

        let cleanResponse = typeof data === 'string' ? data : JSON.stringify(data);
        if (cleanResponse.trim().toLowerCase().startsWith('<!doctype') || cleanResponse.includes('<html')) return null;
        ...
        console.error(`[Pollinations POST Error]: ${postErr.message} (Status: ${postErr.response?.status})`);
        // If rate-limited or failed, try Pollinations GET endpoint (separate rate limit)
        try {
            const systemPrompt = getSystemPrompt(message);
            const encodedMsg = encodeURIComponent(message);
            const encodedSys = encodeURIComponent(systemPrompt.substring(0, 500)); // truncate system to avoid URL length limits
            const url = `https://text.pollinations.ai/${encodedMsg}?system=${encodedSys}&model=openai&seed=${Math.floor(Math.random()*9999)}`;
            const { data } = await axios.get(url, {
                timeout: 12000,
                headers: { 'User-Agent': `Mozilla/5.0 (HamzaBot/${Math.floor(Math.random()*99)+1})` }
            });
            let resp = typeof data === 'string' ? data : JSON.stringify(data);
            if (resp.trim().toLowerCase().startsWith('<!doctype') || resp.includes('<html')) return null;
            resp = resp.replace(/\*Support Pollinations\.AI:\*[\s\S]*$/, '').trim();
            return sanitizeAiResponse(resp);
        } catch (getErr) {
            console.error(`[Pollinations GET Error]: ${getErr.message} (Status: ${getErr.response?.status})`);
            return null;
        }
    }
}
...
async function getStableAIResponse(jid, message) {
    try {
        ...
        console.error(`[StableAI Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}
...
async function getHectormanuelAI(jid, message, model = "gpt-4o") {
    try {
        ...
        console.error(`[Hectormanuel AI Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}
...
// DuckDuckGo AI Chat — Free GPT-4o mini, no key needed
async function getDuckDuckGoAI(jid, message) {
    try {
        ...
        const history = (await getContext(jid)).messages.slice(-6).map(m => ({
        ...
        const chatRes = await axios.post('https://duckduckgo.com/duckchat/v1/chat', {
            ...
            messages: [
                ...history,
                { role: 'user', content: getSystemPrompt(message) + '\n\n' + message }
            ]
        }, {
            headers: {
                'x-vqd-4': vqdToken,
...
// TypeGPT — Free GPT-4o, no key needed
async function getTypeGPTResponse(jid, message) {
    try {
        const history = (await getContext(jid)).messages.slice(-6).map(m => ({
            role: m.role === 'assistant' ? 'assistant' : 'user',
            content: m.content
        ...
        return sanitizeAiResponse(data?.choices?.[0]?.message?.content || null);
    } catch (e) {
        return null;
    }
}

// Nerimity AI — Free Mistral, no key needed
async function getNerimityAI(jid, message) {
    try {
        ...
        return sanitizeAiResponse(data?.response || data?.message || data?.reply || null);
    } catch (e) {
        return null;
    }
}
...
// Pollinations Qwen — separate model pool on Pollinations (different rate limit bucket)
async function getPollinationsQwen(jid, message) {
    try {
        const encodedMsg = encodeURIComponent(message);
        const sysPrompt = encodeURIComponent(getSystemPrompt(message).substring(0, 400));
        const { data } = await axios.get(
            `https://text.pollinations.ai/${encodedMsg}?system=${sysPrompt}&model=qwen-coder&seed=${Math.floor(Math.random() * 99999)}`,
            { timeout: 12000, headers: { 'User-Agent': `Mozilla/5.0 (HamzaBot-Q/${Math.floor(Math.random()*9)+1})` } }
        );
        let resp = typeof data === 'string' ? data : JSON.stringify(data);
        if (resp.trim().toLowerCase().startsWith('<!doctype') || resp.includes('<html') || resp.includes('429')) return null;
        resp = resp.replace(/\*Support Pollinations\.AI:\*[\s\S]*$/, '').trim();
        return sanitizeAiResponse(resp);
    } catch (e) {
        console.error(`[Pollinations Qwen Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}
// Akash Network — Multiple free models, no auth required (CONFIRMED WORKING)
const AKASH_MODELS = [
...
async function getAkashModel(jid, message, model) {
    try {
        const history = (await getContext(jid)).messages.slice(-6).map(m => ({
            role: m.role === 'assistant' ? 'assistant' : 'user',
            content: m.content
        ...
        if (reply) {
            console.log(`[Akash:${model}] ✅ Success`);
            return sanitizeAiResponse(reply);
        }
        return null;
    } catch (e) {
        console.error(`[Akash:${model} Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}

// Race all 4 Akash models in parallel — fastest one wins
async function getAkashAI(jid, message) {
    const results = await Promise.any(
        AKASH_MODELS.map(m => getAkashModel(jid, message, m).then(r => {
...
// Pollinations GET with random seed — separate rate-limit bucket from POST
async function getPollinationsGET(jid, message) {
    try {
        const encoded = encodeURIComponent(message);
        const sys = encodeURIComponent(getSystemPrompt(message).substring(0, 300));
        const seed = Math.floor(Math.random() * 999999);
        const ua = `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/12${seed % 9}.0`;
        const { data } = await axios.get(
            `https://text.pollinations.ai/${encoded}?system=${sys}&model=openai&seed=${seed}`,
            { timeout: 13000, headers: { 'User-Agent': ua } }
        );
        let resp = typeof data === 'string' ? data : JSON.stringify(data);
        if (resp.trim().toLowerCase().startsWith('<!doctype') || resp.includes('<html') || resp.includes('429')) return null;
        resp = resp.replace(/\*Support Pollinations\.AI:\*[\s\S]*$/, '').trim();
        ...
        console.error(`[Pollinations GET Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}

// Unified fallback — kept for backward compatibility
async function getAIChartResponse(jid, message) {
    return await getAkashModel(jid, message, 'Meta-Llama-3-1-8B-Instruct-FP8');
}

// Stub kept for import compatibility
async function getAirforceAI(jid, message) {
    return await getAkashModel(jid, message, 'Meta-Llama-3-2-3B-Instruct');
}

// Stub kept for import compatibility
async function getGithubModelsAI(jid, message) {
    return await getAkashModel(jid, message, 'Meta-Llama-3-3-70B-Instruct');
}


// ─── NEW PROVIDERS FROM SILANA-LITE ──────────────────────────────────────────

// ChatUpAI — Free completions API (api.chatupai.org), no key required
async function getChatUpAI(jid, message) {
    try {
        const history = (await getContext(jid)).messages.slice(-6).map(m => ({
            role: m.role === 'assistant' ? 'assistant' : 'user',
            content: m.content
        ...
        const { data } = await axios.post(
            ...
            { messages },
            { headers: { 'User-Agent': 'ChatUpAI-Client/1.3.0' }, timeout: 14000 }
        );
        const reply = data?.data?.content;
        if (reply) {
            console.log(`[ChatUpAI] ✅ Success`);
            return sanitizeAiResponse(reply);
        }
        return null;
    } catch (e) {
        console.error(`[ChatUpAI Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}

// NowTechAI — Free streaming AI (aichat.nowtechai.com), no key required
async function getNowTechAI(jid, message) {
    try {
        ...
        const systemPrompt = getSystemPrompt(message);
        const history = (await getContext(jid)).messages.slice(-6).map(m => `${m.role === 'assistant' ? 'Assistant' : 'User'}: ${m.content}`).join('\n');
        ...
        const response = await axios.request({
            ...
            data: JSON.stringify({ content: formattedPrompt }),
            responseType: 'stream',
            timeout: 15000
        });
        ...
        return new Promise((resolve) => {
            ...
            response.data.on('end', () => {
                ...
                if (trimmed) {
                    console.log(`[NowTechAI] ✅ Success`);
                    resolve(sanitizeAiResponse(trimmed));
                } else resolve(null);
            });
            response.data.on('error', () => resolve(null));
        });
    } catch (e) {
        console.error(`[NowTechAI Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}

// ChatEverywhere — Free GPT-4 (chateverywhere.app), no key required
async function getChatEverywhereAI(jid, message) {
    try {
        const sysprompt = getSystemPrompt(message);
        const history = (await getContext(jid)).messages.slice(-6).map(m => ({
            role: m.role === 'assistant' ? 'assistant' : 'user',
            content: m.content
        }));
        ...
        const reply = typeof data === 'string' ? data : data?.content || data?.text;
        if (reply && !reply.includes('<!doctype')) {
            console.log(`[ChatEverywhere] ✅ Success`);
            return sanitizeAiResponse(reply);
        }
        return null;
    } catch (e) {
        console.error(`[ChatEverywhere Error]: ${e.message} (Status: ${e.response?.status})`);
        return null;
    }
}
...
const { 
    askGeminiWeb, 
    askGeminiOfficial, 
    askDuckDuckGo: askDuckDuckGoGemini, 
    getSmartAIReply, 
    isValidReply, 
    HAMZA_SYSTEM_PROMPT 
} = require('./gemini');
...
async function getGeminiResponse(jid, prompt, imageBuffer = null, mime = "image/jpeg", customApiKey = null) {
    try {
        // 1. Text-Only Queries: Use high-speed getSmartAIReply (Gemini Web RPC + Official Keys Rotation + DuckDuckGo)
        if (!imageBuffer) {
            ...
            const history = jid && jid !== "system" ? (await getContext(jid)).messages : [];
            const sysPrompt = getSystemPrompt(prompt, false);

            const aiReply = await getSmartAIReply(finalPrompt, {
                systemPrompt: sysPrompt,
                history: history
            });

            if (aiReply && isValidReply(aiReply)) {
                return sanitizeAiResponse(aiReply);
            }
        }

        // 2. Vision Queries (with imageBuffer)
        ...
        for (const k of keys) {
            try {
                ...
                let contents = [];
                if (jid && jid !== "system") {
                    const history = (await getContext(jid)).messages.slice(-6);
                ...
                if (txt && isValidReply(txt)) return sanitizeAiResponse(txt);
            } catch (_) {}
        }
        return null;
    } catch (e) {
