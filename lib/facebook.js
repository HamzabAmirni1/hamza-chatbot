...
async function uploadToFacebookAttachments(buffer, type, pageToken) {
    try {
        ...
        if (type === 'audio') { ext = 'mp3'; mimeType = 'audio/mpeg'; }
        else if (type === 'video') { ext = 'mp4'; mimeType = 'video/mp4'; }
        else if (type === 'file') { ext = 'bin'; mimeType = 'application/octet-stream'; }
...
async function sendFacebookMedia(recipientId, mediaSource, type, caption, pageToken = config.fbPageAccessToken) {
    ...
    if (isBuffer) {
        try {
            ...
            const attachmentId = await uploadToFacebookAttachments(mediaSource, type, pageToken);
            if (attachmentId) {
    ...
    // ── Strategy 2: Send via direct URL (if we have one) ──
    if (url) {
        try {
            await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${pageToken}`, {
                recipient: { id: recipientId },
    ...
    // ── Strategy 3: Upload buffer to public CDN, then send via URL ──
    if (isBuffer) {
        try {
    ...
    // ── Strategy 4: Raw FormData upload (last resort) ──
    if (isBuffer) {
        try {
            let ext = 'jpg';
    ...
    // ── Strategy 5: For audio/video with URL, send as text link ──
