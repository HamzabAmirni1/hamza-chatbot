...
async function simulateGoogleAnalytics(siteUrl, proxyConfig, ua, ref, route = '', isEngagement = false) {
    ...
    const eventName = isEngagement ? 'user_engagement' : 'page_view';

    // Build the GA4 collect URL
    let gaUrl = `https://www.google-analytics.com/g/collect?v=2&tid=${gaId}&cid=${clientId}&en=${eventName}&dl=${encodedUrl}&dr=${encodedRef}&dt=${encodeURIComponent('Hamza Amirni')}&sr=1920x1080&ul=en-us&sid=${sessionId}&sct=1&seg=1`;
    if (isEngagement) {
...
async function simulateEngagement(siteUrl, proxyConfig, ua, ref) {
    try {
        // 0. Fire Google Analytics for the main page
        await simulateGoogleAnalytics(siteUrl, proxyConfig, ua, ref, '');
        ...
        for (const route of pagesToVisit) {
            ...
            await simulateGoogleAnalytics(siteUrl, proxyConfig, ua, `${siteUrl}/`, route);
            ...
            await simulateGoogleAnalytics(siteUrl, proxyConfig, ua, `${siteUrl}/`, route, true);
        }
    } catch (_) { }
}
    ...
    for (const url of uniqueUrls) {
        ...
        while (!success && attempts < 5) {
            ...
            try {
                ...
                success = true;
                visitCounter++;
                console.log(chalk.blue(`[Ad-Booster] 🚀 Visit #${visitCounter} → ${url} (via ${proxyConfig ? proxyConfig.host : 'Direct'}) [Ref: ${new URL(ref).hostname}]`));

                // 2. Simulate engagement (assets + ad tag + routing) if the proxy works
                await simulateEngagement(url, proxyConfig, ua, ref);
            } catch (err) {
