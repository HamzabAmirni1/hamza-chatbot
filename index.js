...
✨ *Active 24/7 on Koyeb* ✨`;
                ...
                if (body && body.match(/^\.(yts|بحث-يوتيوب|chercher)\s+(.+)/i)) {
                    ...
                    try {
                        ...
                        videos.forEach((v, i) => {
                            ...
                            // Only add buttons for the top 3 (WhatsApp limit)
                            if (i < 3) {
                                buttons.push({
                        ...
                        resultText += `\n💡 *طريقة التحميل:* \n- ضغط على الأزرار أسفله\n- أو صيفط رابط الفيديو مباشرة\n- أو اكتب: .video [رابط من القائمة]\n\n⚔️ *${config.botName}*`;

                        try {
                            if (waitMsg) await sock.sendMessage(sender, { delete: waitMsg.key });
                        } catch (e) { }

                        // Send with requested button format (headerType 1)
                        await sock.sendMessage(sender, {
                            text: resultText,
                            buttons: buttons,
                ...
                if (body && body.toLowerCase() === '.tempnum') {
                    ...
                    try {
                        ...
                        // Format for WhatsApp (Top 15 numbers to avoid too long message)
                        let text = `📱 *أرقام وهمية لتفعيل الحسابات (7sim)*\n\n`;
                        results.slice(0, 15).forEach((res, i) => {
                            text += `*${i + 1}.* 🌍 *${res.country}*\n`;
                            text += `   📞 ${res.number}\n`;
                            text += `   📡 ${res.source}\n`;
                            text += `   🔗 ${res.link}\n\n`;
                        });
                        text += `\n💡 *ملاحظة:* ضغط على الرابط باش تشوف الكود اللي وصل.\n⚔️ *${config.botName}*`;
