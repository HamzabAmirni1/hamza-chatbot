...
async function simulateGoogleAnalytics(siteUrl, proxyConfig, ua, ref, route = '', isEngagement = false, lang = 'en-US') {
    ...
    let gaUrl = `https://www.google-analytics.com/g/collect?v=2&tid=${gaId}&cid=${clientId}&en=${eventName}&dl=${encodedUrl}&dr=${encodedRef}&dt=${encodeURIComponent('Hamza Amirni')}&sr=1920x1080&ul=${ul}&sid=${sessionId}&sct=1&seg=1`;
    if (isEngagement) {
    ...
    for (const url of uniqueUrls) {
        ...
        while (!success && attempts < 5) {
            ...
            try {
                ...
                success = true;
                visitCounter++;
                console.log(chalk.blue(`[Ad-Booster] 🚀 Visit #${visitCounter} → ${url} (via ${proxyConfig ? proxyConfig.host : 'Direct'}) [Lang: ${lang.split(',')[0]}]`));
