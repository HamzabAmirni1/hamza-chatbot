...
async function analyzeImage(buffer, mime, prompt) {
    try {
        ...
        // 5. Vreden Gemini Vision (New reliable provider)
        promises.push((async () => {
            const { uploadToCatbox } = require('./media');
            const imageUrl = await uploadToCatbox(buffer);
            if (!imageUrl) throw new Error("Upload failed for Vreden");
            const { data } = await axios.get(`https://api.vreden.my.id/api/ai/gemini-vision?query=${encodeURIComponent(finalPrompt)}&url=${encodeURIComponent(imageUrl)}`, { timeout: 20000 });
            return data.result || data.response || null;
        })());

        // 6. Obito RAW Buffer Vision
        ...
        const racePromise = Promise.any(promises.map((p, i) => p.then(res => {
            ...
            // ⚠️ Language Filter: If user asked in Arabic, reject purely English responses
            // This prevents fast English-only providers from winning the race when Arabic was requested.
            const hasArabicChars = /[\u0600-\u06FF]/.test(res);
            if (lang === 'ar' && !hasArabicChars && res.length > 50) {
                console.log(chalk.yellow(`[Vision] Provider ${i} returned English for Arabic request. Rejecting...`));
                throw new Error("Language mismatch (Expected Arabic)");
            }
