...
async function ytvCommand(sock, chatId, msg, args, helpers, userLang) {
  if (!args[0]) {
  ...
  if (!ytRegex.test(url)) {
    return await sock.sendMessage(chatId, { text: `❌ *Invalid URL!*\n\nPlease provide a valid YouTube link.\n\nExample:\n.ytv https://youtu.be/xxxxx 720` }, { quoted: msg });
  }

  // Use WhatsApp reactions if available, or just text for Telegram
  if (!helpers?.isTelegram) {
    await sock.sendMessage(chatId, { react: { text: "⏳", key: msg.key } });
  }
  await sock.sendMessage(chatId, { text: `⏳ *Processing...*\nFetching video 🎬 in *${quality}* quality, please wait.` }, { quoted: msg });

  const result = await ytdown(url, quality);

  if (!result.status) {
    return await sock.sendMessage(chatId, { text: `❌ *Download Failed!*\n\nCould not fetch the video. The link may be restricted or invalid.\n\nPlease try again.` }, { quoted: msg });
  }

  const { title, channel, thumbnail, duration, video } = result;

  if (!video?.url) {
    return await sock.sendMessage(chatId, { text: `❌ *Video not available* at the requested quality.\n\nTry a lower quality:\n.ytv ${url} 360` }, { quoted: msg });
  }
  ...
  try {
    ...
    return await sock.sendMessage(chatId, { text: `⚠️ *File too large or failed to send.*\n\nTry a lower quality:\n.ytv ${url} 360` }, { quoted: msg });
  }
}

module.exports = ytvCommand;
