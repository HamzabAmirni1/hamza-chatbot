...
// Scaleway Generative APIs — Free Llama 3.1 70B (no key required for anonymous access)
async function getScalewayAI(jid, message) {
    try {
        const history = (await getContext(jid)).messages.slice(-4).map(m => ({
            role: m.role === 'assistant' ? 'assistant' : 'user',
            content: m.content
        }));
        const { data } = await axios.post(
            ...
            },
            {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer SCW_SECRET_KEY' // public demo key
                },
                timeout: 15000
            }
        );
