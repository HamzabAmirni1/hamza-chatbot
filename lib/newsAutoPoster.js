...
async function getCache() {
    try {
        const cache = await db.getCache('news');
        if (cache && Array.isArray(cache)) return { lastTitles: cache };
    } catch(e) {}
    fs.ensureDirSync(DATA_DIR);
    if (!fs.existsSync(NEWS_CACHE_FILE)) return { lastTitles: [] };
    try { return fs.readJsonSync(NEWS_CACHE_FILE); } catch(e) { return { lastTitles: [] }; }
}

async function saveCache(cache) {
    try { await db.setCache('news', cache.lastTitles || []); } catch(e) {}
    try { fs.writeJsonSync(NEWS_CACHE_FILE, cache); } catch(e) {}
}

async function checkAndPostNews() {
    if (config.enableNewsAutoPoster !== 'true') {
        console.log("[newsAutoPoster] News Auto-Poster is disabled.");
        return;
    }
    const hour = moment().tz(config.timezone || 'Africa/Casablanca').hour();
    if (hour < 8 || hour > 22) {
        console.log("[newsAutoPoster] Outside active posting hours (8am - 10pm). Skipping news check.");
        return;
    }
...
function startNewsScheduler() {
    // Check every 3 hours (to prevent FB posting limits)
    setInterval(checkAndPostNews, 3 * 60 * 60 * 1000);
    console.log("[newsAutoPoster] 🗞️ News Auto-Poster Scheduler started (3h interval).");
}
