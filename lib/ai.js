...
function getSystemPrompt(text, isVision = false) {
    ...
    if (isVision) {
        return lang === 'ar' 
            ? "أنت مساعد ذكي خبير في تحليل الصور وحل التمارين. قم بمراجعة الصورة بدقة.\n\n⚠️ قاعدة ذهبية (التطابق اللغوي):\n- إذا كانت الصورة تحتوي على نص أو تمرين، يجب أن تكون الإجابة كاملة وبشكل حصري بنفس لغة ذلك النص (مثلاً: إذا كان الامتحان بالفرنسية، يجب أن تكون الأجوبة والعناوين وكل شيء بالفرنسية فقط).\n- لا تخلط اللغات؛ إذا كان المحتوى أجنبياً، لا تستخدم رؤوس أقلام بالعربية.\n- اجعل التنسيق جميلاً واحترافياً باستخدام الرموز التعبيرية (Emojis) والقوائم المرتبة.\n- إذا كانت الصورة طبيعية، أجب بالعربية/الدارجة." 
            : "You are an expert image analyzer and problem solver. Please review the image carefully.\n\n⚠️ GOLDEN RULE (Language Matching):\n- If the image contains text, exercises, or questions, your ENTIRE response must be in the same language as that text (e.g., if it's a French exam, answer exclusively in French).\n- Do not mix languages. Do not use English headers for foreign content.\n- Format the response beautifully and professionally using emojis, bold text, and clear lists.\n- If the photo is general, reply in English.";
    }
    const langInstruction = lang === 'ar' 
        ? "\n\n⚠️ هام: أجب دائماً باللغة العربية أو الدارجة المغربية." 
        : "\n\n⚠️ IMPORTANT: Always reply in English.";
    return config.systemPromptAI + langInstruction;
}
...
async function getGeminiProxyVision(base64, prompt) {
    try {
...
async function getOpenRouterResponse(jid, text, imageBuffer = null) {
    try {
        ...
        const messages = [
            { role: "system", content: getSystemPrompt(text, !!imageBuffer) },
            ...history,
            { role: "user", content },
        ];
...
async function analyzeImage(buffer, mime, prompt) {
    try {
        ...
        const promises = [];
        const finalPrompt = prompt || "ماذا يوجد في هذه الصورة؟ اشرح بالتفصيل.";
        const lang = detectLanguage(finalPrompt);
        const base64 = buffer.toString("base64");

        // 1. Gemini AI (If configured or using public key)
        const geminiKey = config.geminiApiKey || "AIzaSyB3Q74etnADQ_qSX3OJtzTnteGh-fd4df8";
        promises.push(getGeminiResponse("system", finalPrompt, buffer, mime));
        
        // 2. OpenRouter (If configured - Uses free Gemini Flash Vision)
        if (config.openRouterKey) {
            promises.push(getOpenRouterResponse("system", finalPrompt, buffer));
        }

        // 3. Gemini Proxy (from external script)
        promises.push(getGeminiProxyVision(base64, finalPrompt));

        // 4. NoteGPT & Fallback Upload APIs
        promises.push((async () => {
            console.log(chalk.blue(`[Vision] Starting NoteGPT/Pollinations branch...`));
            
            // Try uploading first for providers that need URLs
            const { uploadToCatbox, uploadToTelegraPh, uploadToTmpfiles } = require('./media');
            let imageUrl = await uploadToTmpfiles(buffer);
            if (!imageUrl) imageUrl = await uploadToCatbox(buffer);
            if (!imageUrl) imageUrl = await uploadToTelegraPh(buffer);
            
            if (imageUrl) {
                console.log(chalk.cyan(`[Vision] Image uploaded to: ${imageUrl}`));
            ...
            // Fallback: Pollinations direct Base64
            try {
                ...
                if (typeof data === 'string' && !data.includes("<!doctype")) return data;
            } catch (e) {}
            
            throw new Error("All image AI branches failed");
        })());
        ...
        // Race them!
        const racePromise = Promise.any(promises.map((p, i) => p.then(res => {
