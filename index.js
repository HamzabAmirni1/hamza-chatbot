...
app.post('/api/fb-fetch-pages', async (req, res) => {
  try {
    const { userToken } = req.body;
    if (!userToken) return res.status(400).json({ success: false, error: 'رمز الوصول للفيسبوك (User Access Token) مطلوب' });
    ...
    if (response.data && Array.isArray(response.data.data)) {
      const pages = response.data.data.map(p => ({
        ...
        category: p.category || 'صفحة فيسبوك',
        picture: p.picture?.data?.url || `https://graph.facebook.com/${p.id}/picture?type=square`
      }));

      if (pages.length === 0) {
        return res.status(404).json({ success: false, error: 'لم يتم العثور على صفحات مرتبطة بهذا الحساب. تأكد من منح الصلاحيات (pages_show_list, pages_messaging)' });
      }

      return res.json({ success: true, pages });
    } else {
      return res.status(400).json({ success: false, error: 'لم يتم استرجاع صفحات من فيسبوك' });
    }
  } catch (e) {
    console.error('[FB Fetch Pages Error]:', e.response?.data || e.message);
    const errDetail = e.response?.data?.error?.message || e.message;
    return res.status(500).json({ success: false, error: `خطأ في جلب الصفحات من فيسبوك: ${errDetail}` });
  }
});
...
app.get('/api/tempmail/generate', (req, res) => {
  try {
    ...
    // catchmail.io supports mailbox names with domain @catchmail.io
    res.json({ ok: true, email: `${username}@catchmail.io` });
  } catch(e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.get('/api/tempmail/messages', async (req, res) => {
  try {
    ...
    const response = await axios.get(`https://api.catchmail.io/api/v1/mailbox?address=${encodeURIComponent(email)}`);
    res.json({ ok: true, messages: response.data.messages || [] });
  } catch(e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.get('/api/tempmail/message', async (req, res) => {
  try {
    ...
    const response = await axios.get(`https://api.catchmail.io/api/v1/message/${id}?mailbox=${encodeURIComponent(email)}`);
    ...
    try {
      ...
      for (const msg of chatUpdate.messages) {
        ...
        // --- PRIORITY 3: TEXT AI (pure text messages only) ---
        {
          ...
          if (!reply) {
            // Priority 1: Gemini Engine (Web RPC + Official Keys Rotation + DuckDuckGo)
            console.log(chalk.cyan(`[AI] Asking Gemini Engine (Web RPC + Official Keys)...`));
            try {
              reply = await getGeminiResponse(sender, body);
              if (reply) console.log(chalk.green(`[AI] Gemini Engine succeeded (${reply.length} chars)`));
            } catch (_) {}

            // Priority 2: OpenRouter if configured
            ...
            // Priority 3: Fallback providers
            if (!reply) {
              console.log(chalk.cyan(`[AI] Racing fallback providers...`));
              try {
                reply = await Promise.race([
                  Promise.any([
                    getChatEverywhereAI(sender, body),
                    getPollinationsResponse(sender, body),
                    getStableAIResponse(sender, body),
                    getBlackboxResponse(sender, body)
                  ].map(p => p.then(res => {
                ...
                if (reply) console.log(chalk.green(`[AI] Fallback succeeded (${reply.length} chars)`));
              } catch (e) {
                console.log(chalk.yellow(`[AI] Fallback failed: ${e.message}`));
              }
            }
